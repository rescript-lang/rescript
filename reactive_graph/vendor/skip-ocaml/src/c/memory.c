...
extern size_t sk_current_usage(void);
...
#ifdef __APPLE__
    fprintf(stderr,
            "Error: persistent heap '%s' already exists. On macOS you must "
            "delete the heap file between runs.\n",
            filename);
...
CAMLprim value oskip_heap_usage(value unit) {
  CAMLparam1(unit);
  size_t usage = sk_current_usage();
  CAMLreturn(caml_copy_int64((int64_t)usage));
}
