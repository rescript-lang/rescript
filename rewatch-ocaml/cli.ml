...
and build_options = {folder: string; prod: bool; features: string list option}
...
let parse argv =
  ...
  let parse_build ~watch args =
    let rec loop folder prod features = function
    | [] ->
      let command = {folder = Option.value folder ~default:"."; prod; features} in
      if watch then Watch command else Build command
    | ("-h" | "--help") :: _ -> Help
    | ("-V" | "--version") :: _ -> Version
    | "--prod" :: rest -> loop folder true features rest
    | "--features" :: value :: rest ->
      ...
      loop folder prod (Some values) rest
    ...
    | ("-v" | "-vv" | "-q" | "-qq" | "--no-timing") :: rest ->
      loop folder prod features rest
    | arg :: _ when String.length arg > 0 && arg.[0] = '-' ->
      raise (Error ("unknown option " ^ arg))
    | arg :: rest -> (
      match folder with
      | None -> loop (Some arg) prod features rest
      | Some _ -> raise (Error "too many folder arguments"))
    in loop None false None args
  in
  match args with
  | "clean" :: rest ->
