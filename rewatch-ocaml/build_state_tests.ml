...
let () =
  ...
  check (Build_state.has_complete_compile_assets a)
    "both compile artifacts form a complete cached compile";
  check (not (Build_state.has_complete_compile_assets b))
    "a missing compile artifact requires compilation";
  check (not (Build_state.dependency_compiled_after b a))
  ...
  check (Build_state.dependency_compiled_after b a)
