...
| `build/packages.rs` | Package resolution and source/module discovery | `build.ml`, `config.ml`, `source.ml`, `project_context.ml` | Package discovery now owns the compilation modules, full cleanup leaf inventory, and GenType directories; canonical resolved identities flow through graph/build traversal, while broader package-state ownership remains split |
| `build/read_compile_state.rs` | One compile-asset inventory for cleanup and freshness | `compile_assets.ml` and `build_state.ml` | The shared inventory supplies CMI/CMT presence and timestamps plus published-AST source locations; remaining AST/output freshness checks still use live filesystem metadata |
