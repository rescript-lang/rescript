...
| `build/clean.rs` | Stale and explicit artifact cleanup | `build_artifacts.ml`, with command traversal in `build.ml` | Present; stale cleanup accepts the shared compile-asset inventory, and recursive inventories avoid duplicate existence/metadata probes while retaining symlink behavior |
