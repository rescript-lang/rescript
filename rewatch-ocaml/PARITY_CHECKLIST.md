...
| Package/dependency graph | Canonical compile/feature cases and graph unit tests; the differential command gate covers missing, config-less, and malformed dependency packages for build/clean/watch with Rust's exit class | Partial; remaining source guards and diagnostics are inventoried below |
