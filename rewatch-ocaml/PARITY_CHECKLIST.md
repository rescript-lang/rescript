...
| `build/clean.rs` | Stale and explicit artifact cleanup | `build_artifacts.ml`, with command traversal in `build.ml` | Present; stale cleanup consumes shared compile-asset and source inventories, avoiding duplicate source-tree walks while retaining symlink behavior; the working `lib/bs` tree is still scanned |
