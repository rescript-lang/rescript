...
| Source filter matching | `build/packages.rs`: `matches_filter`, `read_folders` | `source.ml`: `discover_with_inventory` | Differential builds prove that a directory-only regex excludes a nested source while a basename regex includes it; focused source tests retain both boundaries | Matched for basename rather than package-relative-path matching; cross-engine regex syntax remains under source audit |
