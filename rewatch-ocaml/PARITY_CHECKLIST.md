...
| Parse execution and graph invariants | `build/parse.rs`: `generate_asts`, `generate_ast`; `build/deps.rs`: `get_dep_modules` | `build.ml`: `parse_job`, `ast_dependencies`, global parse scheduling and publication | Canonical syntax-error, warning-persistence, rename, and deletion cases plus the exact compiler-work gate cover normal failures and state transitions; a differential compiler wrapper removes a successfully generated AST before dependency extraction; package/module/namespace lookups are graph-construction invariants in both implementations | Matched for compiler outcomes and internal state; OCaml reports the missing AST normally while Rust panics, and the harder pre-parser source-disappearance race remains source-audited but lacks a deterministic gate |
| Compile execution and dependency scheduling | `build/compile.rs`: scheduler, `compiler_args`, `compile_file`, dirty propagation | `build.ml`: global graph resolution, scheduled compilation, `publish_compiled`; `build_state.ml` | Canonical cycle, missing-module, interface, warning, namespace, feature, and incremental-watch cases; exact clean/unchanged/edit work manifests; fresh-tree artifact equivalence; focused `-bs-no-bin-annot` build | Matched for user-reachable compiler and scheduler outcomes, including optional CMT/CMTI debug artifacts; package/module/interface unwraps are invariants established by the graph and scheduled job shape |
...
| Implicit format project scope and graph validation | `format.rs`: `get_files_in_scope`; `ProjectContext::get_scoped_local_packages`; `packages::make` | `format.ml`: `files_in_scope`, `validate_package_graph`, `local_dependency`, `package_sources`; `build.ml`: package resolution and locality helpers | Four canonical format tests cover the current fixture, a single file, stdin, and formatting from a workspace package; focused integration proves that implicit format requires a config in the current directory rather than searching parents; `format_tests.ml` proves installed `node_modules` dependencies are excluded from formatting; the differential command gate covers missing, config-less, malformed, duplicate-path, omitted-source, and requested feature-cycle dependencies | Matched: the complete applicable dependency graph and restricted feature requests are validated first, while the current package is always formatted, direct symlink-local regular/dev dependencies are included only at a monorepo root, a listed child formats only itself, transitive and installed dependencies are excluded from formatting, and all feature-gated source directories are considered |
