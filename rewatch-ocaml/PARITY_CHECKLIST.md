...
| `build/compiler_info.rs` | Compiler/config fingerprints and package invalidation | `compiler_info.ml` | Compiler, runtime, package config, source-map arguments, and effective root package-output specs invalidate every affected package; previous output specs drive precise stale-output removal, including installed source dependencies |
| `build/clean.rs` | Stale and explicit artifact cleanup | `build_artifacts.ml`, with command traversal in `build.ml` | Present; stale cleanup consumes shared compile-asset/source inventories and directly calculated working paths; a whole-tree `lib/bs` scan is retained only as a lazy malformed/legacy-artifact fallback |
