...
| `build/compile.rs` | Compiler arguments, dirty propagation, scheduling, publication | `compiler_args.ml`, `compiler_scheduler.ml`, `build_state.ml`, `process.ml`, and job construction/publication in `build.ml` | Package-output, namespace, source-map, GenType, and warning arguments have an explicit owner; the dependency-aware interface/implementation state machine, deterministic failure aggregation, cleanup, and byte-identical CMI-change propagation now have an explicit scheduler owner, while job construction and publication/freshness ownership remain to be consolidated |
