...
| Implicit format project scope | `format.rs`: `get_files_in_scope`; `ProjectContext::get_scoped_local_packages`; `packages::make` | `format.ml`: `files_in_scope`, `local_dependency`, `package_sources` | Four canonical format tests cover the current fixture, a single file, stdin, and formatting from a workspace package | Partial; source-level equivalence for transitive/local dependency scope and discovery failures remains |
