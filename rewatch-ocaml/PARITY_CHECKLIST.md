...
| CLI and format input validation | Dedicated Cmdliner tests mirror Rust CLI cases, including required/surplus `compiler-args` paths; compiler-args tests cover extension, dependency selection, and missing-package behavior; focused format failures cover stdin labels and check summaries; differential cases exactly compare explicit missing-file, directory, and unsupported-extension failures and retain the stale-compiler Rust panic fix; canonical format/compiler-args cases cover success | CLI shape, format input validation, explicit-file filesystem failures, and compiler discovery are matched or documented; remaining implicit project-scope diagnostics stay in the source inventory |
