  ...
  with_temp_dir (fun root ->
    ...
    let result =
      Build_artifacts.cleanup_stale
        ...
        ~source_files:[public_output; public_map] ~root ~ocaml_dir ~is_local:true
        config []
    in
    List.iter
      (fun path ->
        check (not (Sys.file_exists path))
          ("direct cleanup should remove " ^ path))
      [public_output; public_map; working_ast; working_output; working_map; published_ast; published_cmi];
    check (Sys.file_exists working_cmi)
      "a directly mapped working CMI remains available through compilation";
    check (result.deferred_artifacts = [working_cmi])
      "direct cleanup returns the working CMI for deferred removal";
    check (result.removed_modules = ["Old"])
  ...
  with_temp_dir (fun root ->
    ...
    ignore
      (Build_artifacts.cleanup_stale ~ocaml_files:[published_cmt]
         ...
         config []);
    check (not (Sys.file_exists working_cmt))
