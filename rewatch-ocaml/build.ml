...
type attempt_request =
  ...
  | Full_watch_attempt of retained_build option
...
let carry_forward_compile_dirtiness previous next_session next_state =
...
let run_with_warning_state ~poll ~warning_state ~request ~no_timing ~verbosity
  ...
  let execute ~release_build_lock =
    ...
    | Full_watch_attempt (Some previous) ->
      carry_forward_compile_dirtiness previous attempt.session
        prepared.build_state
