...
### Debugging Binary AST Marshalling Issues

The Rust compiler must produce byte-identical binary AST output to OCaml's `output_value` (Marshal format). This section documents how to debug parity issues in the binary marshalling layer.

#### How Binary Marshalling Works

OCaml's `output_value` serializes values to a binary format with:
- **Block headers** - Encode tag and size (e.g., `0xB0` = block with tag 0, size 3)
- **Shared references** - Previously-written objects can be referenced by offset
- **Integers** - Small ints (0-63) are single bytes, larger use multi-byte encoding
- **Strings** - Length-prefixed byte sequences

The Rust implementation is in `compiler-rust/src/binary_ast/`:
- `marshal.rs` - Low-level Marshal format writing
- `serialize.rs` - Generic `Marshal` trait for types
- `types.rs` - Marshal impl for Location, Position, Located<T>
- `current_marshal.rs` - Marshal impls for current parsetree types

#### Identity-Based vs Content-Based Sharing

**This is the most common source of parity issues.**

OCaml shares objects based on **pointer identity** - if two values are the same object in memory (same pointer), they share. If they're different objects with identical content, they do NOT share.

The Rust implementation mimics this with `LocationId` and `PositionId`:
- Each `mk_loc()` call generates a unique `LocationId`
- When a Location is **cloned**, the ID is preserved (same identity)
- During marshalling, Locations with the same ID share

**Common Bug Pattern**: Using `.clone()` on a Location when OCaml would create separate objects.

```rust
// WRONG - Both locations share the same ID, OCaml wouldn't share these
let loc = p.mk_loc(&start_pos, &end_pos);
let expr = Expression { pexp_loc: loc.clone(), ... };
let item = StructureItem { pstr_loc: loc, ... };

// CORRECT - Each gets a unique ID, matching OCaml's behavior
let expr = Expression { pexp_loc: p.mk_loc(&start_pos, &end_pos), ... };
let item = StructureItem { pstr_loc: p.mk_loc(&start_pos, &end_pos), ... };
```

#### Debugging Steps for Binary Parity Issues

1. **Generate hex dumps of both outputs**:
   ```bash
   # Generate binary ASTs
   ./compiler-rust/target/release/bsc -bs-ast /tmp/test.res -o /tmp/rust.ast
   ./packages/@rescript/darwin-arm64/bin/bsc.exe -bs-ast /tmp/test.res -o /tmp/ocaml.ast
...
2. **Check file sizes** - If Rust is smaller, it's likely over-sharing; if larger, under-sharing:
   ```bash
   ls -la /tmp/rust.ast /tmp/ocaml.ast
   ```

3. **Identify the divergence point** - Find the first byte that differs:
   ```bash
   cmp -l /tmp/rust.ast /tmp/ocaml.ast | head -10
   ```

4. **Decode the Marshal format** at the divergence:
   - `0x40` = int 0
   - `0x80-0xBF` = small block: `byte = 0x80 + tag + (size << 4)`
   - `0x04 NN` = shared reference to object #NN
   - `0xB0` = block(tag=0, size=3) which is a Location

5. **Check LocationId assignments** in the Rust AST:
   ```bash
   ./compiler-rust/target/release/res_parser_rust --print ast --typechecker /tmp/test.res 2>&1 \
     | perl -0777 -ne 'print "$1 " while /id: LocationId\(\s*(\d+)/sg'
   ```
   Look for duplicate IDs where there shouldn't be any.

#### Example: Fixing Location Sharing Bug

**Symptom**: Rust output 4 bytes smaller than OCaml. At offset 0x98:
- OCaml: `b0 04 03 04 02 40` (new Location block with shared positions)
- Rust: `04 03` (shared reference to existing Location)

**Diagnosis**: LocationId sequence showed `1 2 3 3 4 5...` - duplicate ID 3.

**Root Cause**: In `typ.rs parse_attribute_payload()`:
```rust
let loc = p.mk_loc(&p.start_pos, &p.end_pos);  // Creates ID 3
p.next();
let expr = Expression { pexp_loc: loc.clone(), ... };  // Uses ID 3
let item = StructureItem { pstr_loc: loc, ... };       // Uses ID 3 again!
```

**Fix**: Use separate `mk_loc()` calls:
```rust
let start_pos = p.start_pos.clone();
let end_pos = p.end_pos.clone();
p.next();
let expr = Expression { pexp_loc: p.mk_loc(&start_pos, &end_pos), ... };  // ID 3
let item = StructureItem { pstr_loc: p.mk_loc(&start_pos, &end_pos), ... };  // ID 4
```

#### Testing Binary Parity

```bash
# Test a single file
./compiler-rust/target/release/bsc -bs-ast /tmp/test.res -o /tmp/rust.ast
./packages/@rescript/darwin-arm64/bin/bsc.exe -bs-ast /tmp/test.res -o /tmp/ocaml.ast
diff /tmp/rust.ast /tmp/ocaml.ast && echo "MATCH" || echo "DIFFERS"

# Test many files
./scripts/test_parser_ast_parity.sh  # Includes binary parity test
```

#### Marshal Format Quick Reference

| Byte Range | Meaning |
|------------|---------|
| `0x00-0x3F` | Small positive integer (0-63) |
| `0x40-0x7F` | Small negative integer (-64 to -1) |
| `0x80-0xBF` | Small block: tag = (byte-0x80) & 0x0F, size = (byte-0x80) >> 4 |
| `0x04 NN` | CODE_SHARED8: shared ref to object at distance NN |
| `0x05 NN NN` | CODE_SHARED16: shared ref with 16-bit distance |
| `0xC0-0xDF` | Small string: length = byte - 0xC0 |

#### Common Marshal Patterns

- **Location** (3 fields): `0xB0` header, then start Position, end Position, ghost int
- **Located<T>** (2 fields): `0xA0` header, then txt, loc
- **Empty list**: `0x40` (int 0)
- **Cons cell** (2 fields): `0xA0` header, then element, tail
