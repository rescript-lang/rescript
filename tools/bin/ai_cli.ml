...
Usage: %s lint <FILE-OR-DIR> [--config <FILE>] [--json]
...
Example: %s lint ./src/MyModule.res|}
...
Usage: %s rewrite <FILE-OR-DIR> [--config <FILE>] [--diff] [--json]
...
Example: %s rewrite ./src/MyModule.res|}
    banner prog_name prog_name
...
Usage: %s active-rules <FILE-OR-DIR> [--config <FILE>] [--json]
...
Usage: %s show <SYMBOL-PATH> [--kind <auto|module|value|type>] [--context <FILE-OR-DIR>] [--comments <include|omit>]
...
Example: %s show String.localeCompare|}
    banner prog_name prog_name
...
Usage:
  %s find-references <SYMBOL-PATH> [--kind <auto|module|value|type>] [--context <FILE-OR-DIR>]
  %s find-references --file <FILE> --line <LINE> --col <COL>
...
Example: %s find-references String.localeCompare|}
...
let is_ai_command command = List.mem command command_names

let moved_command_message ~prog_name command =
  Printf.sprintf
    "error: `%s` has moved to the standalone `%s` binary.\n\nUse:\n  %s %s"
    command prog_name prog_name command
...
let main ~prog_name ~version () =
  ...
  | "lint" :: rest -> (
    ...
    | _ -> log_and_exit (Error (lint_help prog_name)))
  | "rewrite" :: rest -> (
    match rest with
    ...
    | path :: args -> (
      let rec parse_args config_path json diff = function
        ...
        | _ -> Error (rewrite_help prog_name)
      in
      ...
      | Ok (config_path, json, diff) -> (
        ...
        | Ok {Tools.Rewrite.output; _} ->
          if output <> "" then print_endline output;
          exit 0))
    | _ -> log_and_exit (Error (rewrite_help prog_name)))
  | "active-rules" :: rest -> (
    match rest with
