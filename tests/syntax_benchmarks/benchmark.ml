...
module Benchmarks : sig
  ...
  let parse_rescript src filename =
    ...
    let structure = Res_parser1.parse_implementation p in
  ...
  let benchmark (filename, action) =
    ...
    let benchmark_fn =
      ...
      | Print ->
        ...
        let ast = Res_parser1.parse_implementation p in
        fun () ->
          let _ =
            Sys.opaque_identity
              (let cmt_tbl = Comment_table.make () in
               let comments = List.rev p.Parser.comments in
               let () = Comment_table.walk_structure ast cmt_tbl comments in
