...
@genType let origin = {x: 0, y: 0, z: Some(0)}
...
@genType let someBusiness = {name: "SomeBusiness", owner: None, address: None}
...
@genType let getPayload = ({payload}) => payload
...
@genType let getPayloadRecord = ({payload}): record => payload

@genType let recordValue = {v: 1, w: 1}

@genType let payloadValue = {num: 1, payload: recordValue}
...
@genType type mix = {"a": int, "b": int, "c": option<{"name": string, "surname": string}>}
...
@genType type myObj = {"type_": string}

@genType let testMyRec = (x: myRec) => x.type_

@genType let testMyRec2 = (x: myRec) => x

@genType let testMyObj = (x: myObj) => x["type_"]

@genType let testMyObj2 = (x: myObj) => x
...
@genType let testMyRecBsAs2 = (x: myRecBsAs) => x
