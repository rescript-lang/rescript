...
function kill(collid, ctx) {
  switch (collid.TAG) {
    ...
    case "Enemy" :
      ...
      return Stdlib_List.concat(score, remains);
    case "Item" :
      let o$1 = collid._2;
  ...
  switch (prob) {
    ...
    case 2 :
      if (blockh - cby === 1) {
        ...
        return Stdlib_List.concat(four, Stdlib_List.concat(three, Stdlib_List.concat(two, one)));
      } else {
        return /* [] */0;
      }
    case 3 :
      if (stair_typ === 0 && blockh - cby > 3) {
        ...
        return Stdlib_List.concat(three$1, Stdlib_List.concat(two$1, one$1));
        ...
        return Stdlib_List.concat(one$2, Stdlib_List.concat(two$2, three$2));
      } else {
        return {
          hd: [
            stair_typ,
...
function generate_enemies(blockw, blockh, _cbx, _cby, acc) {
  while (true) {
    ...
    if (prob < 3 && blockh - 1 === cby) {
      ...
      return Stdlib_List.concat(enemy, generate_enemies(blockw, blockh, cbx, cby + 1, acc));
    }
    _cby = cby + 1;
    continue;
  };
...
function generate_block_locs(blockw, blockh, _cbx, _cby, _acc) {
  while (true) {
    ...
    if (prob < 5) {
      ...
      let called_acc = Stdlib_List.concat(acc, undup_lst);
      _acc = called_acc;
      _cby = cby + 1;
      continue;
    }
...
function generate_ground(blockw, blockh, _inc, _acc) {
  while (true) {
    ...
    if (inc > 10) {
      let skip = int(10);
      let newacc = Stdlib_List.concat(acc, {
        hd: [
          4,
          [
            inc * 16,
    ...
    let newacc$1 = Stdlib_List.concat(acc, {
      hd: [
        4,
        [
          inc * 16,
...
function generate_helper(blockw, blockh, cx, cy, context) {
  ...
  let block_locations = Stdlib_List.concat(block_locs, ground_blocks);
  let all_blocks = Stdlib_List.concat(obj_converted_block_locs, obj_converted_ground_blocks);
  ...
  let converted_block_coin_locs = Stdlib_List.concat(converted_block_locs, coin_locs);
  ...
  return Stdlib_List.concat(all_blocks, Stdlib_List.concat(obj_converted_enemies, Stdlib_List.concat(coin_objects, Stdlib_List.concat(obj_enemy_blocks, {
    hd: obj_panel,
    tl: /* [] */0
  }))));
}
