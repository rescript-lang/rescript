...
describe(__MODULE__, () => {
  test("basic set operations", () => {
    ...
    ok(__LOC__, !N.isEmpty(u7))
    ok(__LOC__, N.isEmpty(u8))
    ok(__LOC__, N.has(u10, 20))
    ok(__LOC__, N.has(u10, 21))
    ...
    ok(__LOC__, N.has(u15, 20000))
    ok(__LOC__, !N.has(u15, 2000))
    ok(__LOC__, N.isEmpty(u16))
  })
  ...
  test("set union and intersection operations", () => {
    ...
    ok(__LOC__, N.eq(u28, u29))
    ok(__LOC__, N.toArray(u29) == S.stableSortBy(A.concat(ss, [3]), compare))
    ok(__LOC__, N.eq(u19, u20))
    eq(__LOC__, N.toArray(u21), I.range(59, 100))
    eq(__LOC__, N.toArray(u22), I.range(0, 58))
    ok(__LOC__, N.eq(u24, u19))
    eq(__LOC__, N.toArray(u23), I.range(101, 200))
    ok(__LOC__, N.subset(u23, u18))
  ...
  test("set iteration and comparison operations", () => {
    ...
    let u2 = N.add(u1, 33)
    ok(__LOC__, L.every2(testIterToList(u0), L.makeBy(21, i => i), (x, y) => x == y))
    ok(__LOC__, L.every2(testIterToList2(u0), L.makeBy(21, i => i), (x, y) => x == y))
    ok(__LOC__, L.every2(testIterToList(u0), N.toList(u0), (x, y) => x == y))
    ok(__LOC__, N.some(u0, x => x == 17))
    ok(__LOC__, !N.some(u1, x => x == 17))
    ok(__LOC__, N.every(u0, x => x < 24))
    ok(__LOC__, D.every(N.getData(u0), x => x < 24))
    ok(__LOC__, !N.every(u2, x => x < 24))
    ok(__LOC__, !N.every(N.fromArray(~id=module(IntCmp), [1, 2, 3]), x => x == 2))
    ok(__LOC__, N.cmp(u1, u0) < 0)
    ok(__LOC__, N.cmp(u0, u1) > 0)
  })

  test("set keep and partition operations", () => {
    ...
    throws(__LOC__, _ => ignore(N.getExn(a0, 1002)))
    throws(__LOC__, _ => ignore(N.getExn(a0, -1)))
    eq(__LOC__, N.size(a0), 1001)
    ok(__LOC__, !N.isEmpty(a0))
  ...
  test("empty set operations", () => {
    ...
    ok(__LOC__, N.isEmpty(N.keep(a, x => mod(x, 2) == 0)))
  })

  test("split empty set operations", () => {
