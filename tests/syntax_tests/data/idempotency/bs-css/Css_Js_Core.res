...
let countersReset = xs => D("counter-reset", xs->Belt.Array.map(string_of_counter_reset)->join(" "))

let counterSet = x => D("counter-set", string_of_counter_set(x))
let countersSet = xs => D("counter-set", xs->Belt.Array.map(string_of_counter_set)->join(" "))
