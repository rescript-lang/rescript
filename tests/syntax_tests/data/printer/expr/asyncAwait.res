...
let inBinaryExpression = await x->Promise.resolve + 1
let inBinaryExpression = await x->Promise.resolve + await y->Promise.resolve

let withCallback = async (()) => {
    async (x) => await (x->Promise.resolve) + 1
}
...
let _ = await f(x)
let _ = await f(x)
let _ = await (f(x) : Promise.t<unit>)
