    ...
    let (expr, attrs_consumed) = match &p.token {
        ...
        Token::Lident(s) if s == "async" => {
            ...
            if super::core::is_es6_arrow_expression(p, context == ExprContext::TernaryTrueBranch) {
                ...
                (
                    parse_es6_arrow_expression(p, true, Some(start_pos), attrs.clone()),
                    true,
                )
            } else {
                // Just a regular "async" identifier
                let loc = mk_loc(&start_pos, &p.prev_end_pos);
                (
                    ast_helper::make_ident(Longident::Lident("async".to_string()), loc),
                    false,
                )
            }
        }
        ...
        _ => {
            let is_arrow =
                super::core::is_es6_arrow_expression(p, context == ExprContext::TernaryTrueBranch);
            if is_arrow {
                // Pass attrs to arrow expression - they're consumed
                (
                    parse_es6_arrow_expression(p, false, None, attrs.clone()),
                    true,
                )
            } else {
...
fn parse_parameter(p: &mut Parser<'_>) -> Option<super::core::FundefParameter> {
    ...
    if p.token == Token::Tilde {
        ...
        let (label, default) = if p.token == Token::Equal {
            p.next();
...
fn parse_payload(p: &mut Parser<'_>) -> Payload {
    // Simplified payload parsing - just parse as string for now
    // TODO: Implement full payload parsing
    let mut depth = 1;
...
pub fn parse_atomic_expr(p: &mut Parser<'_>) -> Expression {
    ...
    let expr = match &p.token {
        ...
        Token::Percent => {
            // Extension expression: %ext or %ext(payload)
            p.next();
        ...
        Token::Underscore => {
            p.err(DiagnosticCategory::Message(
                "Unexpected underscore".to_string(),
            ));
            p.next();
            recover::default_expr()
        }
...
pub fn parse_constrained_or_coerced_expr(p: &mut Parser<'_>) -> Expression {
    ...
    if p.token == Token::Colon {
        ...
        if p.token == Token::ColonGreaterThan {
            ...
            Expression {
                pexp_desc: ExpressionDesc::Pexp_coerce(Box::new(expr), Some(typ), target_typ),
                pexp_loc: loc,
                pexp_attributes: vec![],
            }
        } else {
    ...
    loop {
        match &p.token {
            ...
            Token::Lbracket => {
                ...
                if p.token == Token::Equal {
                    // Array assignment: arr[index] = value -> Array.set(arr, index, value)
                    p.next();
                    let value = parse_expr(p);
                    ...
                    // Array access: arr[index] -> Array.get(arr, index)
                    // Mark with attribute so printer can restore bracket syntax
                    let loc = mk_loc(&start, &p.prev_end_pos);
            ...
            Token::MinusGreater => {
                // Pipe operator: a->f(b) becomes f(a, b)
                p.next();

                // Parse the right-hand side - could be:
                // - a simple identifier: a->f
                // - a function call: a->f(b)
                // - a parenthesized expression: a->(x => x+1)
                // - a module path: a->Mod.f(b)
                // - a block expression: a->switch b { ... }, a->if c { ... }

                let rhs = if p.token == Token::Lparen {
                    // Parenthesized expression: a->(f) or a->(x => x+1) or a->(f: type)
                    p.next();
                    let inner = parse_constrained_or_coerced_expr(p);
                    p.expect(Token::Rparen);
                    inner
                } else if matches!(
                    p.token,
                    Token::Switch | Token::If | Token::For | Token::While | Token::Try
                ) {
                    // Block expression like switch, if, for, while, try
                    parse_atomic_expr(p)
                } else {
                ...
                // Check if followed by function call args
                if p.token == Token::Lparen {
                    let mut args = parse_call_args(p);
                    args.insert(0, (ArgLabel::Nolabel, expr.clone()));
                    let loc = mk_loc(&expr.pexp_loc.loc_start, &p.prev_end_pos);
                    expr = ast_helper::make_apply(rhs, args, loc);
                } else {
                    // Just a->f, becomes f(a)
                    let loc = mk_loc(&expr.pexp_loc.loc_start, &p.prev_end_pos);
                    expr = ast_helper::make_apply(rhs, vec![(ArgLabel::Nolabel, expr)], loc);
                }
            }
            Token::Backtick if !no_call => {
                ...
                // expr must be an identifier for tagged templates
                match &expr.pexp_desc {
                    ...
                    _ => {
                        // Non-identifier tag - emit error but still parse
                        p.err(DiagnosticCategory::Message(
                            "Tagged template literals are currently restricted to names like: myTagFunction`foo ${bar}`.".to_string(),
                        ));
...
fn parse_call_args(p: &mut Parser<'_>) -> Vec<(ArgLabel, Expression)> {
    let mut args = vec![];
    ...
    let _uncurried = p.token == Token::Dot;
    if _uncurried {
        p.next();
    }
...
fn parse_array_expr(p: &mut Parser<'_>) -> Expression {
    ...
    let mut _spread = None;
    while p.token != Token::Rbracket && p.token != Token::Eof {
        if p.token == Token::DotDotDot {
            // Array spread: [...arr, elem]
            p.next();
            _spread = Some(Box::new(parse_expr(p)));
            if !p.optional(&Token::Comma) {
                break;
            }
            continue;
    ...
    // For now, just output the elements and ignore spread
    // TODO: Properly handle spread in arrays
    Expression {
...
/// Parse a list expression list{e1, e2, ...}.
/// Note: The Token::List already includes the '{', so we don't expect Lbrace.
fn parse_list_expr(p: &mut Parser<'_>, start_pos: Position) -> Expression {
    ...
    while p.token != Token::Rbrace && p.token != Token::Eof {
        if p.token == Token::DotDotDot {
            p.next();
            _spread = Some(Box::new(parse_expr(p)));
            // Allow trailing comma after spread
    ...
    // Build the list from elements and spread
    // For proper representation, we wrap elements in a tuple and spread in a separate structure
    // The printer will handle these specially for res.list
    let payload_items: Vec<StructureItem> = elements
        .into_iter()
        .map(|e| StructureItem {
            pstr_desc: StructureItemDesc::Pstr_eval(e, vec![]),
            pstr_loc: loc.clone(),
        })
        .collect();
    let payload = Payload::PStr(payload_items);

    // If there's a spread, we need to encode it - use a special attribute name
    let ext_name = if _spread.is_some() {
        "res.list.spread".to_string()
    } else {
        "res.list".to_string()
    };

    let spread_attrs = if let Some(spread) = _spread {
        // Store spread as an attribute
        vec![(
            mknoloc("res.spread".to_string()),
            Payload::PStr(vec![StructureItem {
                pstr_desc: StructureItemDesc::Pstr_eval(*spread, vec![]),
                pstr_loc: loc.clone(),
            }]),
        )]
    } else {
    ...
    Expression {
        pexp_desc: ExpressionDesc::Pexp_extension((mknoloc(ext_name), payload)),
        pexp_loc: loc,
        pexp_attributes: spread_attrs,
    }
}
...
fn parse_dict_expr(p: &mut Parser<'_>, start_pos: Position) -> Expression {
    ...
    let mut _fields: Vec<(String, Expression)> = vec![];
    while p.token != Token::Rbrace && p.token != Token::Eof {
        // Parse key (string)
        match &p.token {
            Token::String(s) => {
    ...
    // Simplified: just create an extension node
    Expression {
...
fn parse_braced_or_record_expr(p: &mut Parser<'_>) -> Expression {
    ...
    let is_record = p.lookahead(|state| {
        if let Token::Lident(_) = &state.token {
            ...
            matches!(
                state.token,
                Token::Colon | Token::Comma | Token::Rbrace | Token::Question
            )
        } else {
            false
        }
...
fn parse_block_body(p: &mut Parser<'_>) -> Option<Expression> {
    ...
    // Attach any leading attributes to the expression
    if !attrs.is_empty() {
    ...
    while p.token != Token::Rbrace && p.token != Token::Eof {
        match &p.token {
            Token::Lident(name) => {
                ...
                let (lid, expr) = if p.token == Token::Colon {
                    // field: value
                    p.next();
                    let expr = parse_expr(p);
                    let loc = mk_loc(&field_start, &p.prev_end_pos);
                    (with_loc(Longident::Lident(name), loc), expr)
                } else {
                    // Punning: field is same as variable
                    let loc = mk_loc(&field_start, &p.prev_end_pos);
                    let expr = ast_helper::make_ident(Longident::Lident(name.clone()), loc.clone());
                    (with_loc(Longident::Lident(name), loc), expr)
                };
...
fn parse_first_class_module_expr_inner(p: &mut Parser<'_>, start_pos: Position) -> Expression {
    let mod_expr = super::module::parse_module_expr(p);
...
fn parse_try_expr(p: &mut Parser<'_>) -> Expression {
    ...
    while p.token != Token::Rbrace && p.token != Token::Eof {
        if p.token == Token::Bar {
            ...
            p.expect(Token::EqualGreater);
            // Use SwitchCaseRhs context to stop at `|` (case delimiter)
            let rhs = parse_expr_with_context(p, ExprContext::SwitchCaseRhs);
            cases.push(Case {
                pc_bar: bar_pos,
                pc_lhs: lhs,
                pc_guard: guard,
