...
- Lower exception packing (`Pwrap_exn`) in `translcore` instead of convert, drop unused `raise_kind` / reraise tracking, and emit `RE_EXN_ID` string equality from matching instead of `Pextension_slot_eq`.
