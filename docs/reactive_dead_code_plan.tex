...
\begin{itemize}[leftmargin=*]
  ...
  \item Set \code{Summary.version = 2} (reflecting the absolute-position schema); bump the constant whenever new mandatory fields ship and regenerate cached artifacts.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item surface an \code{-incremental-liveness} CLI flag so developers can build the graph, observe the recompute frontier, and keep it in lockstep with batch diagnostics even before the Skip runtime hosts it.
\end{itemize}

We treat the graph solver as experimental behind the \code{INCR\_GRAPH\_SOLVER=1} environment flag: by default the CLI builds/prints the frontier but reuses the batch liveness answers so parity can run on every change. As soon as the SCC hashing + heuristics catch up, flipping the flag enables the Tarjan-based recompute without touching the driver loop.
...
For now the \code{-incremental-liveness} flag drives a “shadow” path: summaries feed the in-process \code{Graph_store}, we log the size of the recompute frontier after every batch run, and the parity harness executes the same two-stage flow on curated fixtures. Once the Skip service lands we will replace the logging with actual incremental diagnoses, but keeping the flag in tree de-risks each intermediate refactor.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Stage 2 of the parity harness now builds the graph, prints the incremental frontier, and compares incremental-vs-legacy deaths. Developers can flip \code{INCR\_GRAPH\_SOLVER} or \code{INCR\_AFTER\_LEGACY} in that command to experiment with the pure solver without regressing the default behaviour.
  \item Log intermediate graph/SCC hashes in the implementation log so we can trace regressions.
\end{itemize}

\subsubsection*{Milestone 3.1 – Annotation Semantics}
Thread \code{ProcessDeadAnnotations.(annotateLive|annotateDead|doReportDead)} through the graph solver so declarative annotations (React components, @dead, etc.) behave exactly like the batch path. Finishing this milestone should remove the current block of “unexpected deaths” involving annotated React props/components.

\subsubsection*{Milestone 3.2 – Delayed Edge Replay}
Ensure \code{DeadOptionalArgs.forceDelayedItems} and the exception replay run before we query the graph, so optional-arg and ghost exception references populate the edge set just like in batch mode. This should shrink the “missing deaths” set where optional parameters currently appear unused.

\paragraph{Status.} Completed.
\begin{itemize}[leftmargin=*]
  \item Added \code{DeadException.replay\_delayed\_items} and \code{DeadOptionalArgs.replay\_delayed\_items}; these re-run the delayed-edge logic against any \emph{pure} collector without consuming the shared queues. \code{Reanalyze.loadCmtFile} now calls them only when a summary/graph run is needed (\code{-cache-summaries} or \code{-incremental-liveness}), so batch runs never touch the new code.
  \item Taught \code{Collector.collected} to deep-copy the mutable optional-argument payloads before storing \code{Common.decl}s. Summary-side replays can now mark arguments as used without mutating the legacy \code{DeadCommon} structures coming from the sink collector.
  \item Removed the previous attempt that flushed \code{DeadException}/\code{DeadOptionalArgs} queues at the end of every \code{DeadCode.processCmt}. The queues now drain once per batch run (as before); the summary path reads them through the replay helpers without disturbing the batch data.
\end{itemize}

\paragraph{Validation.}
\begin{itemize}[leftmargin=*]
  \item \code{make test-analysis}
  \item \code{dune exec analysis/bin/collector\_parity.exe -- tests/analysis_tests/tests-reanalyze/deadcode/lib/bs}
    \begin{itemize}
      \item Stage 1 (collector parity) remains clean.
      \item Stage 2 (\code{INCR\_GRAPH\_SOLVER=1}) still reports 115 “unexpected deaths”; those correspond to the remaining sub-milestones (file ordering, reference normalisation, module bookkeeping).
    \end{itemize}
\end{itemize}

\subsubsection*{Milestone 3.3 – File Ordering Parity}
Carry the legacy \code{orderedFiles} rank into each node and order SCC processing by that rank, preventing premature deaths when dependencies live in other files. Completing this step should eliminate mismatches caused purely by traversal order.

\paragraph{Status.} Completed.
\begin{itemize}[leftmargin=*]
  \item \code{Graph_store} now retains the per-file dependency DAG emitted by the collector and recomputes the same topological order that \code{DeadCommon.iterFilesFromRootsToLeaves} derives. Every time summaries update file edges or digests we invalidate and recompute the rank table so it stays in sync with the legacy ordering.
  \item The incremental solver fetches those ranks, sorts the frontier before feeding it to Tarjan, and sorts every SCC’s member list with \code{Decl.compareUsingDependencies}, mirroring the batch traversal (files processed from leaves to roots, then bottom-to-top inside each file).
\end{itemize}

\paragraph{Validation.}
\begin{itemize}[leftmargin=*]
  \item \code{make test-analysis}
  \item \code{INCR\_GRAPH\_SOLVER=1 dune exec analysis/bin/collector\_parity.exe -- tests/analysis_tests/tests-reanalyze/deadcode/lib/bs}
    \begin{itemize}
      \item Stage 1 remains clean.
      \item Stage 2 still reports exactly 115 unexpected deaths, all tracked for milestones 3.4–3.5. No file-order-related regressions remain.
    \end{itemize}
\end{itemize}

\subsubsection*{Milestone 3.4 – Reference Normalisation}
Apply the same location rewriting (\code{Current.lastBinding}, ghost filtering, point locations) before counting incoming references so self-ref edges and ghost locations do not skew the solver. Expect the long tail of prop/record mismatches to drop after this milestone.

\paragraph{Status.} Completed.
\begin{itemize}[leftmargin=*]
  \item \code{Graph_store} no longer drops references whose source position does not map to a known declaration. Instead it tracks those as ``unknown live'' edges (mirroring the legacy \code{ValueReferences} behaviour) and carries the flag through pending-resolution paths so targets stay live until the legacy solver would mark them dead.
  \item \code{Incremental_liveness.resolve} consults the new flag before evaluating a declaration, ensuring any call-site we could not attribute to a concrete declaration still prevents the target from being killed. This eliminated the spurious kills for JSX props and optional-arg helpers whose call sites originate from generated code.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \code{INCR\_GRAPH\_SOLVER=1 dune exec analysis/bin/collector\_parity.exe -- tests/analysis_tests/tests-reanalyze/deadcode/lib/bs}
    \begin{itemize}
      \item Stage 1 remains clean.
      \item Stage 2 now reports 110 unexpected deaths (down from 115), all in the categories reserved for Milestone 3.5.
    \end{itemize}
\end{itemize}

\subsubsection*{Milestone 3.5 – Reactive Remediation Playbook}
\noindent\textbf{Why it exists.} Milestone 3 introduced the incremental graph and Tarjan solver, but it also surfaced a class of cross-file regressions (missing keepalive edges, `.resi`/\ `.res` mismatches, and orphaned pending queues). Milestone 3.5 collects the remediation techniques we applied so far and documents them as a repeatable playbook for future reactive migrations.

\paragraph{Remediation steps.}
\begin{enumerate}[leftmargin=*]
  \item \textbf{Normalise sources and targets.} Always record value references under the physical file where the expression originated, even if a different summary emitted the reference. Likewise, collapse `.resi` keys to their `.res` twin before adding pending entries so interface/implementation pairs drain the same queue.
  \item \textbf{Replay pending edges deterministically.} Augment the graph store with bucketed path queues (\code{path_bucket}, \code{path_components_match}) and explicit tracing so we can prove that deferred edges are replayed when declarations finally appear. This mirrors the legacy `DeadCommon` “unknown live” semantics while keeping instrumentation lightweight.
  \item \textbf{Tighten lookup heuristics.} Require literal file matches for range/line/name scans before falling back to global indices; otherwise cross-file lookups tempt fate by binding to the wrong declaration. When those heuristics fail, enqueue an unknown edge instead of silently dropping the reference.
  \item \textbf{Keep module bookkeeping hooked up.} Every SCC resolution still calls \code{DeadModules.markDead/markLive}. That ensures higher-level diagnostics (e.g. nested React components) stay synchronised while we experiment with incremental liveness underneath.
\end{enumerate}

\paragraph{Reference guidance for future conversions.}
\begin{itemize}[leftmargin=*]
  \item Treat cross-file aliases as first-class citizens: synthesise target paths directly from compiler metadata, and prefer matching declarations by canonical path before trying positional heuristics.
  \item Instrument aggressively (\code{GRAPH\_TRACE\_*}) while the incremental solver is experimental, but gate the logs behind env vars so the debugging aids can live in-tree without spamming CI.
  \item When a regression reproduces only after multiple files load (e.g. `DeadValueTest.valueAlive`), capture both the `.res` and `.resi` summaries in the log; the next engineer should never have to rediscover the repro.
\end{itemize}

\paragraph{Current status (suspended).} The remediation work is on hold while we focus on other milestones. Values now resolve correctly across module boundaries, but type propagation and alias replay are still incomplete, so the `DeadValueTest.valueAlive` mismatch remains as our sentinel. All helper traces and documentation stay in place so the effort can resume later without context loss.

\paragraph{Evaluation checkpoint (2025-11-17).} Re-running the documented parity harness (\code{dune exec analysis/bin/collector\_parity.exe -- tests/analysis_tests/tests-reanalyze/deadcode/lib/bs}) exposed a Stage~1 regression (missing value references in \texttt{ErrorHandler.cmt}), contradicting earlier claims that the collector stage was clean. Several Milestone~3 fixes (synthetic top-level \texttt{\_} declarations for every \texttt{Tstr\_eval}, variant dependency mirroring, etc.) now execute inside the legacy analyzer, so our ``original'' results have drifted and we no longer have three comparable datasets (original, modified legacy, incremental). Stage~2 parity still falls back to the legacy solver unless \code{INCR\_GRAPH\_SOLVER=1} is exported, meaning the reported incremental improvements merely show that the modified legacy pipeline agrees with itself. Combined with the lack of measured performance data, this evaluation led us to pause Milestone~3 work and move on until we can (1) restore Stage~1 parity, (2) freeze a trustworthy legacy baseline, and (3) demonstrate actual incremental performance wins.

\paragraph{Validation to date.}
\begin{itemize}[leftmargin=*]
  \item \code{dune build analysis/reanalyze} and \code{make test-analysis} (ensuring no batch regressions while we diagnose incremental gaps).
  \item Targeted parity runs such as \code{INCR\_GRAPH\_SOLVER=1 GRAPH\_TRACE\_UNKNOWN=1 dune exec analysis/bin/collector\_parity.exe -- DeadValueTest tests/analysis_tests/tests-reanalyze/deadcode/lib/bs}, which now highlight only the suspended type/alias discrepancies.
\end{itemize}

Each mini-milestone targets a single discrepancy category; running the parity harness after each completion should show a strictly decreasing mismatch count.
Each step removes a distinct block of discrepancies that the parity harness currently reports, letting us watch the diff set shrink monotonically as we implement the missing behaviour.
