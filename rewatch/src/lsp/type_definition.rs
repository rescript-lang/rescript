...
use crate::lsp::{analysis, definition};

/// Handle a type definition request: resolve the source buffer, lock build state,
...
/// Run a type definition request by shelling out to `rescript-editor-analysis.exe rewatch typeDefinition`.
///
...
#[instrument(name = "lsp.type_definition", skip_all, fields(
...
fn run(
    ...
    {
        let _guard = tracing::info_span!("lsp.type_definition.ensure_cmt").entered();
    ...
    let json_blob = {
        let _guard = tracing::info_span!("lsp.type_definition.build_context").entered();
    ...
    let _guard = tracing::info_span!("lsp.type_definition.analysis_binary").entered();

    let stdout = analysis::spawn_analysis_binary(build_state, &["rewatch", "typeDefinition"], &json_blob)?;
