(* Representation of ['a t]:
   ...
   - Data slots: queue elements, stored as ['a ReactiveAllocator.offheap].
...
let head = ReactiveAllocator.Block2.get0
let set_head = ReactiveAllocator.Block2.set0
let tail = ReactiveAllocator.Block2.get1
let set_tail = ReactiveAllocator.Block2.set1
...
let resize (type a) (t : a t) new_cap =
  ...
  let fresh =
    ReactiveAllocator.Block2.create ~capacity:new_cap ~x0:0 ~y0:old_len
  in
  for i = 0 to old_len - 1 do
    let src = slot_index t (head t + i) in
    ReactiveAllocator.Block2.set fresh i (ReactiveAllocator.Block2.get t src)
  done;
  ReactiveAllocator.Block2.blit ~src:fresh ~src_pos:0 ~dst:t ~dst_pos:0
    ~len:new_cap;
  ReactiveAllocator.Block2.destroy fresh
...
let push t x =
  ...
  ReactiveAllocator.Block2.set t (slot_index t tail_i) x;
  set_tail t (tail_i + 1)

let pop t =
  if is_empty t then invalid_arg "ReactiveFifo.pop: empty";
  let head_i = head t in
  let x = ReactiveAllocator.Block2.get t (slot_index t head_i) in
  set_head t (head_i + 1);
  x
