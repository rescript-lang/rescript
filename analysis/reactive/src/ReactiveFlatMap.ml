(** Zero-allocation (steady-state) flatMap state and processing logic.
    ...
    provenance, iter_with for all iterations). *)
...
type ('k1, 'v1, 'k2, 'v2) t = {
  ...
  target: ('k2, 'v2) ReactiveHash.Map.t;
  (* Scratch — allocated once, cleared per process() *)
  scratch: ('k1, 'v1 ReactiveMaybe.t) ReactiveHash.Map.t;
  affected: 'k2 ReactiveHash.Set.t;
...
let add_single_contribution (t : (_, _, _, _) t) k2 v2 =
  ...
  ReactiveHash.Set.add t.affected k2

(* Emit callback for init — writes directly to target *)
let add_single_contribution_init (t : (_, _, _, _) t) k2 v2 =
  ReactivePoolMapSet.add t.provenance t.current_k1 k2;
  ReactivePoolMapMap.replace t.contributions k2 t.current_k1 v2;
  ReactiveHash.Map.replace t.target k2 v2

let create ~f ~merge ~output_wave =
  let rec t =
    {
      f;
      merge;
      ...
      scratch = ReactiveHash.Map.create ();
...
let push t k v_opt = ReactiveHash.Map.replace t.scratch k v_opt
...
let recompute_target (t : (_, _, _, _) t) k2 =
  if ReactivePoolMapMap.inner_cardinal t.contributions k2 > 0 then (
    ...
    ReactiveHash.Map.replace t.target k2 t.merge_acc;
    ReactiveWave.push t.output_wave k2 (ReactiveMaybe.some t.merge_acc))
  else (
    ReactiveHash.Map.remove t.target k2;
    ReactiveWave.push t.output_wave k2 ReactiveMaybe.none)
...
let process (t : (_, _, _, _) t) =
  ...
  ReactiveHash.Map.iter_with process_scratch_entry t t.scratch;
  ReactiveHash.Map.clear t.scratch;
