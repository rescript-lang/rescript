...
let create ~initial_capacity : 'a t =
  ...
  Allocator.Block.set t length_slot (Obj.magic (Allocator.int_to_offheap 0));
  t
...
let push (t : 'a t) value =
  ...
  Allocator.Block.set t length_slot
    (Obj.magic (Allocator.int_to_offheap next_len))
...
let pop (t : 'a t) =
  ...
  Allocator.Block.set t length_slot
    (Obj.magic (Allocator.int_to_offheap (len - 1)));
  last
