...
let apply_left_entry t k mv =
  ...
  if Maybe.is_some mv then (
    ReactiveMap.replace t.left_values (Stable.unsafe_of_value k)
      (Stable.unsafe_of_value (Maybe.unsafe_get mv));
    r.adds_received <- r.adds_received + 1)
  else (
    ReactiveMap.remove t.left_values (Stable.unsafe_of_value k);
    r.removes_received <- r.removes_received + 1);
  ReactiveSet.add t.affected (Stable.unsafe_of_value k)
...
let apply_right_entry t k mv =
  ...
  r.entries_received <- r.entries_received + 1;
  if Maybe.is_some mv then (
    ReactiveMap.replace t.right_values (Stable.unsafe_of_value k)
      (Stable.unsafe_of_value (Maybe.unsafe_get mv));
    r.adds_received <- r.adds_received + 1)
  else (
    ReactiveMap.remove t.right_values (Stable.unsafe_of_value k);
    r.removes_received <- r.removes_received + 1);
  ReactiveSet.add t.affected (Stable.unsafe_of_value k)
...
let recompute_affected_entry t k =
  let k = Stable.unsafe_to_value k in
  let r = t.result in
  let lv = ReactiveMap.find_maybe t.left_values (Stable.unsafe_of_value k) in
  let rv = ReactiveMap.find_maybe t.right_values (Stable.unsafe_of_value k) in
  ...
  if has_left then (
    if has_right then (
      ...
      in
      ReactiveMap.replace t.target (Stable.unsafe_of_value k)
    ...
    ReactiveMap.remove t.target (Stable.unsafe_of_value k);
