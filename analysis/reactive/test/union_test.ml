...
let test_union_with_merge () =
  ...
  emit_left (Set ("x", Int_set.of_list [1; 2]));
  let v = get combined "x" |> Option.get in
  Printf.printf "After left Set(x, {1,2}): {%s}\n"
    (Int_set.elements v |> List.map string_of_int |> String.concat ", ");
  assert (Int_set.equal v (Int_set.of_list [1; 2]));

  (* Add to right: key "x" -> {3, 4} (should merge) *)
  emit_right (Set ("x", Int_set.of_list [3; 4]));
  let v = get combined "x" |> Option.get in
  Printf.printf "After right Set(x, {3,4}): {%s}\n"
    (Int_set.elements v |> List.map string_of_int |> String.concat ", ");
  assert (Int_set.equal v (Int_set.of_list [1; 2; 3; 4]));

  (* Update left: key "x" -> {1, 5} *)
  emit_left (Set ("x", Int_set.of_list [1; 5]));
  let v = get combined "x" |> Option.get in
  Printf.printf "After left update to {1,5}: {%s}\n"
    (Int_set.elements v |> List.map string_of_int |> String.concat ", ");
  assert (Int_set.equal v (Int_set.of_list [1; 3; 4; 5]));
  ...
  Printf.printf "After right Remove(x): {%s}\n"
    (Int_set.elements v |> List.map string_of_int |> String.concat ", ");
  assert (Int_set.equal v (Int_set.of_list [1; 5]));
