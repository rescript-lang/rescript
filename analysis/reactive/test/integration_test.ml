...
let test_file_collection () =
  ...
  let word_counts =
    flat_map ~name:"word_counts" files
      ~f:(fun _path counts -> String_map.bindings counts)
  ...
  let counts_a =
    String_map.empty |> String_map.add "hello" 2 |> String_map.add "world" 1
  in
  let counts_b =
    String_map.empty |> String_map.add "hello" 1 |> String_map.add "foo" 1
  in
  ...
  Printf.printf "\nModifying file_a...\n";
  let counts_a' =
    String_map.empty |> String_map.add "hello" 1 |> String_map.add "world" 2
  in
  emit_file (Set ("file_a", counts_a'));
