...
and completion_type =
  | Tuple of Query_env.t * Types.type_expr list * Types.type_expr
  | Texn of Query_env.t
  | Tpromise of Query_env.t * Types.type_expr
  | Toption of Query_env.t * inner_type
  | Tresult of {
      env: Query_env.t;
  ...
  | TinlineRecord of {env: Query_env.t; fields: field list}
  | Tfunction of {
      env: Query_env.t;
...
module Env = struct
  type t = {stamps: Stamps.t; module_path: Module_path.t}
...
type extra = {
  ...
  file_references: (string, Location_set.t) Hashtbl.t;
...
let all_files_in_package package =
  File_set.union package.project_files package.dependencies_files
...
module Completion = struct
  type kind =
    ...
    | FollowContextPath of Completable.context_path * Scope_types.item list
