...
type resolution =
  | Exported of Query_env.t * file_path
...
let rec make_path ~(env : Query_env.t) module_path =
  match module_path with
...
and find_in_module ~(env : Query_env.t) module_ path =
  match module_ with
  | Structure structure ->
    resolve_path_inner ~env:(Query_env.enter_structure env structure) ~path
