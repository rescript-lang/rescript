...
module Server = struct
  ...
  type server_state = {
    ...
    stats: server_stats;
    mutable config_snapshot: Run_config.snapshot;
  }
...
|}
          ...
          Ok
            {
              ...
              config_snapshot = Run_config.snapshot ();
            })
  ...
  let run_one_request (state : server_state) (_req : request) :
    ...
    let file_stats : Reactive_analysis.processing_stats =
      ...
      with_cwd
        ...
        state.config.cwd (fun () ->
          capture_stdout_stderr (fun () ->
              ...
              let new_snapshot = Run_config.snapshot () in
              if
                not
                  (Run_config.equal_snapshot state.config_snapshot new_snapshot)
              then (
