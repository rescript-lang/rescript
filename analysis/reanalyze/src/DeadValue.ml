...
let check_any_value_binding_with_no_side_effects ~config ~decls ~file
  ...
  | Tpat_any when (not (SideEffects.check_expr expr)) && not loc.loc_ghost ->
    ...
    let path = module_path.path @ [FileContext.module_name_tagged file] in
    name
 ...
 fun super self pat ->
  ...
  | Typedtree.Tpat_record (cases, _clodsedFlag) ->
    ...
    |> List.iter (fun (_loc, {Types.lbl_loc = {loc_start = pos_to}}, _pat, _) ->
           if !Config.analyze_types then
             DeadType.add_type_reference ~config ~refs ~pos_from ~pos_to)
  | _ -> ());
    ...
    if !Config.analyze_types then
      ...
      let manifest_type_path =
        ...
        | Some {desc = Tconstr (path, _, _)} -> (
          let p = path |> DcePath.from_path_t in
          match p with
          | [type_name] ->
            let module_context =
        ...
        | _ -> None
      in
      DeadType.add_declaration ~config ~decls ~file ~module_path ~type_id:id
  ...
  | Sig_value (id, {Types.val_loc = loc; val_kind = kind; val_type})
    ...
    if not loc.Location.loc_ghost then
      ...
      if (not is_primitive) || !Config.analyze_externals then
        let optional_args =
          val_type |> DeadOptionalArgs.from_type_expr |> OptionalArgs.from_list
        in
                  ...
                  |> List.iter
                       (fun (type_declaration : Typedtree.type_declaration) ->
                         ...
                         let manifest_type_path =
                           ...
                           | _ -> None
                         in
                         DeadType.add_declaration ~config ~decls ~file
...
let process_value_dependency ~config ~decls ~refs ~file_deps ~cross_file
  ...
  if (not ghost1) && (not ghost2) && pos_to <> pos_from then (
    ...
    DeadOptionalArgs.add_function_reference ~config ~decls ~cross_file ~loc_from
      ~loc_to)
