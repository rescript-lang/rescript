...
let missing_raise_info_to_text {Issue.missing_annotations; loc_full} =
  ...
  in
  if !Cli.json then
    Emit_json.emit_annotate ~action:"Add @throws annotation"
      ~pos:(Emit_json.loc_to_pos loc_full)
...
let log_issue ~config ~(issue : Issue.t) =
  ...
  if config.Dce_config.cli.json then
    ...
    Format.asprintf "%a%s%s"
      (fun ppf () ->
        Emit_json.emit_item ~ppf ~name:issue.name
          ~kind:
      ...
      (if config.Dce_config.cli.json then Emit_json.emit_close () else "")
  else
    let color =
      match issue.severity with
      | Warning -> Color.info
...
module Stats = struct
  ...
  let report ~config =
    ...
    if not config.Dce_config.cli.json then (
