...
let find_externally_referenced ~(decl_store : Declaration_store.t)
    ~(refs : References.t) : bool Pos_hash.t =
  let externally_referenced = Pos_hash.create 256 in

  (* Helper: check if posFrom is a declaration position *)
  let is_decl_pos pos_from =
    Declaration_store.find_opt decl_store pos_from <> None
  in
  ...
  References.iter_value_refs_from refs (fun pos_from pos_to_set ->
      if not (is_decl_pos pos_from) then
        Pos_set.iter
          (fun pos_to -> Pos_hash.replace externally_referenced pos_to true)
          pos_to_set);
...
let is_root ~ann_store ~externally_referenced (decl : Decl.t) =
  ...
  || Pos_hash.mem externally_referenced decl.pos
...
let build_decl_refs_index ~(decl_store : Declaration_store.t)
    ~(refs : References.t) : (Pos_set.t * Pos_set.t) Pos_hash.t =
  let index = Pos_hash.create 256 in

  (* Group declarations by file for efficient lookup *)
  let decls_by_file : (string, (Lexing.position * Decl.t) list) Hashtbl.t =
    Hashtbl.create 256
  in
  Declaration_store.iter
    (fun pos decl ->
  ...
  let add_targets decl_pos targets ~is_type =
    let value_targets, type_targets =
      match Pos_hash.find_opt index decl_pos with
      | Some pair -> pair
      | None -> (Pos_set.empty, Pos_set.empty)
    in
    let new_pair =
      if is_type then (value_targets, Pos_set.union type_targets targets)
      else (Pos_set.union value_targets targets, type_targets)
    in
...
let compute_forward ~debug ~(decl_store : Declaration_store.t)
    ~(refs : References.t) ~(ann_store : Annotation_store.t) :
    live_reason Pos_hash.t * (Pos_set.t * Pos_set.t) Pos_hash.t =
  let t0 = Unix.gettimeofday () in
  let live = Pos_hash.create 256 in
  ...
  if debug then (
    ...
    Log_.item "  decls: %d@."
      (Declaration_store.fold (fun _ _ acc -> acc + 1) decl_store 0);
  ...
  Declaration_store.iter
    (fun pos decl ->
      if is_root ~ann_store ~externally_referenced decl then (
        ...
        in
        Pos_hash.replace live pos reason;
  ...
  while not (Queue.is_empty worklist) do
    ...
    if not (Annotation_store.is_annotated_dead ann_store pos) then
      ...
      | Some (value_targets, type_targets) ->
        ...
        Pos_set.iter
          (fun target ->
            if not (Pos_hash.mem live target) then
              match Declaration_store.find_opt decl_store target with
              | Some target_decl
                when not (target_decl.decl_kind |> Decl.Kind.is_type) ->
                incr propagated_count;
                Pos_hash.replace live target Propagated;
  ...
  if !Cli.timing then
    Printf.eprintf
      "  Liveness.compute_forward: %.3fms (roots=%d, propagated=%d, live=%d)\n\
       %!"
      ((t1 -. t0) *. 1000.0)
      !root_count !propagated_count (Pos_hash.length live);
