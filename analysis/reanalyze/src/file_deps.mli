...
val create : files:File_set.t -> deps:File_set.t File_hash.t -> t
(** Create a FileDeps.t from files set and deps hashtable *)

(** {2 Read-only API for t - for analysis} *)

val get_files : t -> File_set.t
(** Get all files. *)

val get_deps : t -> string -> File_set.t
(** Get files that a given file depends on. *)

val iter_deps : t -> (string -> File_set.t -> unit) -> unit
