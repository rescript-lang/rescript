...
let process_attributes ~(scope_default : scope_default) ~state ~config
  ...
  | Some File_annotations.GenType -> File_annotations.annotate_gentype state pos
  | None -> ());
  ...
  let name_is_in_live_names_or_paths () =
    config.Dce_config.cli.live_names |> List.mem name
    ||
    let fname =
...
let collect_export_locations ~state ~config ~do_gen_type =
  ...
  let scope_default_from_toplevel_attribute (attribute : Parsetree.attribute) :
    ...
    in
    if get_payload "dead" <> None then Some File_annotations.Dead
    else if get_payload "live" <> None then Some File_annotations.Live
    else if get_payload "genType" <> None then Some File_annotations.GenType
    else None
  in
