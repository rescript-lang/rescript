...
let try_request_default ~cwd ~argv : response option =
  let socket_dir, socket_path = default_socket_for_current_project_exn () in
  try_request ~socket_dir:(Some socket_dir) ~socket_path ~cwd ~argv
...
|}
  ...
  let run_one_request (state : server_state) (req : request) : response =
    ...
    let connect () =
      ...
      Fun.protect
        ~finally:(fun () ->
          ...
          close_in_noerr ic)
        (fun () ->
          let req : request = {cwd; argv = Array.of_list reanalyze_args} in
          Marshal.to_channel oc req [Marshal.No_sharing];
          flush oc;
          let (resp : response) = Marshal.from_channel ic in
