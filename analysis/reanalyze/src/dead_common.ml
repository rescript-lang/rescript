...
let solve_dead_forward ~ann_store ~config ~decl_store ~refs ~optional_args_state
  ...
  let incoming_decl_deps : Pos_set.t Pos_hash.t =
    if not debug then Pos_hash.create 0
    else
      let incoming = Pos_hash.create 256 in
      let add_incoming ~target ~source =
        let existing =
          match Pos_hash.find_opt incoming target with
          | Some s -> s
          | None -> Pos_set.empty
        in
        Pos_hash.replace incoming target (Pos_set.add source existing)
      in
      Pos_hash.iter
        (fun source_pos (value_targets, type_targets) ->
          let add_targets targets =
            Pos_set.iter
              (fun target_pos ->
                match Declaration_store.find_opt decl_store target_pos with
...
let solve_dead ~ann_store ~config ~decl_store ~ref_store ~optional_args_state
  ...
  match Reference_store.get_refs_opt ref_store with
  | Some refs ->
