...
let values_builder_add (builder : values_builder) ~module_path ~name exceptions
    =
  let path = (name |> Name.create) :: module_path.Module_path.path in
  Hashtbl.replace builder (path |> Dce_path.to_name) exceptions
...
module Values = struct
  let get_from_module (table : values_table) ~module_name ~module_path
      (path_ : Dce_path.t) =
    let name = path_ @ module_path |> Dce_path.to_name in
...
module Event = struct
  type kind =
    ...
    | Call of {callee: Dce_path.t; module_path: Dce_path.t} (* foo() *)
    | DoesNotThrow of
  ...
  let combine ~(values_table : values_table) ~config ~module_name events =
    ...
    let extend_exn_table exn loc =
      ...
      | Some loc_set -> Hashtbl.replace exn_table exn (Loc_set.add loc loc_set)
      | None -> Hashtbl.replace exn_table exn (Loc_set.add loc Loc_set.empty)
    in
    ...
    let rec loop exn_set events =
      ...
      | ({kind = Throws; exceptions; loc} as ev) :: rest ->
        if config.Dce_config.cli.debug then Log_.item "%a@." print ev;
      ...
      | ({kind = Call {callee; module_path}; loc} as ev) :: rest ->
        ...
        let exceptions =
          ...
          | _ -> (
            match Exn_lib.find callee with
            | Some exceptions -> exceptions
      ...
      | ({kind = DoesNotThrow nested_events; loc} as ev) :: rest ->
        ...
        (if Exceptions.is_empty nested_exceptions (* catch-all *) then
           let name =
             ...
             | {kind = Call {callee}} :: _ -> callee |> Dce_path.to_name
...
let traverse_ast ~file ~values_builder ~checks_builder () =
  ...
  let find_local_exceptions ~module_path path =
    let name = path @ module_path |> Dce_path.to_name in
  ...
  let expr ~(module_path : Module_path.t) (self : Tast_mapper.mapper)
    ...
    | Texp_ident (callee_, _, _) ->
      let callee =
        callee_ |> Dce_path.from_path_t |> Module_path.resolve_alias module_path
      in
      let callee_name = callee |> Dce_path.to_name in
  ...
  let toplevel_eval (self : Tast_mapper.mapper) (expr : Typedtree.expression)
    ...
    current_id := name;
    current_events := [];
    let module_name = file.File_context.module_name in
  ...
  in
  let value_binding ~(module_path : Module_path.t) (self : Tast_mapper.mapper)
  ...
  in
  let make_mapper (module_path : Module_path.t) : Tast_mapper.mapper =
  ...
  in
  let rec process_module_expr (module_path : Module_path.t)
  ...
  and process_structure (module_path : Module_path.t)
      (structure : Typedtree.structure) =
    let rec loop (mp : Module_path.t) (items : Typedtree.structure_item list) =
      match items with
      | [] -> ()
      | structure_item :: rest ->
        ...
        let mp' =
          ...
          | Tstr_module {mb_id; mb_loc; mb_expr} -> (
            ...
            let mp_inside = Module_path.enter_module mp ~name ~loc:mb_loc in
            process_module_expr mp_inside mb_expr;
            match mb_expr.mod_desc with
            | Tmod_ident (path_, _lid) ->
              Module_path.add_alias mp ~name
                ~path:(path_ |> Dce_path.from_path_t)
            | _ -> mp)
          | Tstr_recmodule mbs ->
            ...
            List.fold_left
              (fun acc {Typedtree.mb_id; mb_loc; mb_expr} ->
                let name = mb_id |> Ident.name |> Name.create in
                let mp_inside =
                  Module_path.enter_module acc ~name ~loc:mb_loc
                in
                process_module_expr mp_inside mb_expr;
                match mb_expr.mod_desc with
                | Tmod_ident (path_, _lid) ->
                  Module_path.add_alias acc ~name
                    ~path:(path_ |> Dce_path.from_path_t)
                | _ -> acc)
              mp mbs
          | _ ->
  ...
  fun (structure : Typedtree.structure) ->
    process_structure Module_path.initial structure
...
let process_cmt ~file (cmt_infos : Cmt_format.cmt_infos) : file_result option =
  ...
  | Implementation structure ->
    ...
    Some
      {
        module_name = file.File_context.module_name;
        values_builder;
