...
let create ~(merged : Reactive_merge.t) : t =
  ...
  let value_refs_from : (Lexing.position, Pos_set.t) Reactive.t =
  ...
  let decl_refs_index =
    Reactive_decl_refs.create ~decls ~value_refs_from ~type_refs_from
  in
  ...
  let edges : (Lexing.position, Lexing.position list) Reactive.t =
    Reactive.flat_map ~name:"liveness.edges" decl_refs_index
      ~f:(fun pos (value_targets, type_targets) ->
        let all_targets = Pos_set.union value_targets type_targets in
        [(pos, Pos_set.elements all_targets)])
      ()
  in
  ...
  let annotated_roots : (Lexing.position, unit) Reactive.t =
    Reactive.join ~name:"liveness.annotated_roots" decls annotations
      ...
      ~f:(fun pos _decl ann_opt ->
        match ann_opt with
        | Some File_annotations.Live | Some File_annotations.GenType ->
          [(pos, ())]
        | _ -> [])
      ~merge:(fun () () -> ())
      ()
