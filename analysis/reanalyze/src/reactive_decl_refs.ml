...
let create ~(decls : (Lexing.position, Decl.t) Reactive.t)
    ...
    (Lexing.position, Pos_set.t * Pos_set.t) Reactive.t =
  ...
  let value_decl_refs : (Lexing.position, Pos_set.t) Reactive.t =
  ...
  let type_decl_refs : (Lexing.position, Pos_set.t) Reactive.t =
  ...
  let with_value_refs : (Lexing.position, Pos_set.t) Reactive.t =
    Reactive.join ~name:"decl_refs.with_value_refs" decls value_decl_refs
      ~key_of:(fun pos _decl -> pos)
      ~f:(fun pos _decl refs_opt ->
        [(pos, Option.value refs_opt ~default:Pos_set.empty)])
      ()
  in

  let with_type_refs : (Lexing.position, Pos_set.t) Reactive.t =
  ...
  Reactive.join ~name:"decl_refs.combined" with_value_refs with_type_refs
    ...
    ~f:(fun pos value_targets type_targets_opt ->
      let type_targets = Option.value type_targets_opt ~default:Pos_set.empty in
