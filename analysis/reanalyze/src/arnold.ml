...
module Function_args = struct
  type arg = {label: string; function_name: Function_name.t}
  type t = arg list
...
module Function_call = struct
  type t = {function_name: Function_name.t; function_args: Function_args.t}
  ...
  let apply_substitution ~(sub : Function_args.t) (t : t) =
    if sub = [] then t
    else
      {
  ...
  let to_string {function_name; function_args} =
    function_name ^ Function_args.to_string function_args

  let compare (x1 : t) x2 =
    let n = compare x1.function_name x2.function_name in
    if n <> 0 then n
    else Function_args.compare x1.function_args x2.function_args
end

module Function_call_set = Set.Make (Function_call)
...
module Trace = struct
  ...
  let rec to_string trace =
    ...
    | Tcall (FunctionCall function_call, progress) ->
      Function_call.to_string function_call ^ ":" ^ Progress.to_string progress
    | Tnondet traces ->
...
module Function_table = struct
  ...
  type t = (Function_name.t, function_definition) Hashtbl.t
...
module Check_expression_well_formed = struct
  let traverse_expr ~config ~function_table ~value_bindings_table =
    let super = Tast_mapper.default in
    let check_ident ~path ~loc =
      if path |> Function_table.is_in_function_in_table ~function_table then
...
module Compile = struct
  type ctx = {
    config: Dce_config.t;
    current_function_name: Function_name.t;
    function_table: Function_table.t;
    inner_recursive_functions: (Function_name.t, Function_name.t) Hashtbl.t;
  ...
  let rec expression ~ctx (expr : Typedtree.expression) =
    ...
    | Texp_apply
      ...
      let callee, args =
        ...
        | Some inner_function_name ->
          let inner_function_definition =
            function_table
            |> Function_table.get_function_definition
      ...
      in
      if callee |> Function_table.is_in_function_in_table ~function_table then
        let function_name = Path.name callee in
        let function_definition =
          function_table
          |> Function_table.get_function_definition ~function_name
        in
        ...
        let get_function_arg {Kind.label} =
          ...
          let function_arg () =
            match
              arg_opt
              |> Extend_function_table.extract_labelled_argument
            ...
            | Some (path, _pos)
              ...
              {Function_args.label; function_name}
            | Some (path, _pos)
              when function_table
        ...
        | Some kind when kind = Kind.empty ->
          Command.Call
            (FunctionCall (Path.name callee |> Function_call.no_args), loc)
          |> eval_args ~args ~ctx
        | Some _kind ->
    ...
    | Texp_let
      ...
      function_table
      |> Function_table.add_function ~function_name:new_function_name;
...
module Call_stack = struct
  ...
  type t = {tbl: (Function_call.t, frame) Hashtbl.t; mutable size: int}

  let create () = {tbl = Hashtbl.create 1; size = 0}

  let to_set {tbl} =
    Hashtbl.fold
      (fun frame _i set -> Function_call_set.add frame set)
      tbl Function_call_set.empty
...
module Eval = struct
  type progress = Progress.t
  type cache = (Function_call.t, State.t) Hashtbl.t
  ...
  let update_cache ~config ~function_call ~loc ~state (cache : cache) =
    Stats.log_result ~config ~function_call
      ~res_string:(state |> State.to_string) ~loc;
               ...
               message =
                 Format.asprintf "%a"
                   (fun ppf () ->
                     ...
                     Format.fprintf ppf "@,%a" Call_stack.print call_stack)
                   ();
             });
        Stats.log_loop ());
      true)
  ...
  let rec run_function_call ~config ~cache ~call_stack ~function_args
      ~function_table ~made_progress_on ~loc ~state function_call_to_instantiate
      : State.t =
    let pos = loc.Location.loc_start in
    let function_call =
      function_call_to_instantiate
      |> Function_call.apply_substitution ~sub:function_args
    in
    ...
    let state_after_call =
      ...
      | None ->
        if Function_call_set.mem function_call made_progress_on then
        ...
        then {state with trace = Trace.Tcall (call, state.progress)}
          ...
          let function_definition =
            ...
            |> Function_table.get_function_definition ~function_name
          in
          call_stack |> Call_stack.add_function_call ~function_call ~pos;
          let body =
        ...
        | c :: next_commands ->
          ...
          let made_progress_on, call_stack =
            ...
            | Progress ->
              ...
              ( Function_call_set.union made_progress_on
                  (call_stack |> Call_stack.to_set),
                Call_stack.create () )
  ...
  let analyze_function ~config ~cache ~function_table ~loc function_name =
    if config.Dce_config.cli.debug then
      Log_.log "@[<v 2>@,@{<warning>Termination Analysis@} for @{<info>%s@}@]@."
        function_name;
    let pos = loc.Location.loc_start in
    let call_stack = Call_stack.create () in
    let function_args = Function_args.empty in
    let function_call = Function_call.no_args function_name in
    call_stack |> Call_stack.add_function_call ~function_call ~pos;
    let function_definition =
      function_table |> Function_table.get_function_definition ~function_name
    in
    if function_definition.kind <> Kind.empty then
      ...
      let state =
        body
        |> run ~config ~cache ~call_stack ~function_args ~function_table
             ~made_progress_on:Function_call_set.empty ~state:(State.init ())
      in
...
let traverse_ast ~config ~value_bindings_table =
  ...
  let value_bindings (self : Tast_mapper.mapper) (rec_flag, value_bindings) =
          ...
          |> List.fold_left
               (fun (progress_functions, functions_to_analyze)
                 ...
                 | Some new_progress_functions ->
                   ( String_set.union
                       (String_set.of_list new_progress_functions)
                       progress_functions,
    ...
    if functions_to_analyze <> [] then (
      let function_table = Function_table.create () in
      let is_progress_function path =
        String_set.mem (Path.name path) progress_functions
      in
      ...
      |> Hashtbl.iter
           (fun
             function_name
             (function_definition : Function_table.function_definition)
           ->
             if function_definition.body = None then
               ...
               | Some (_pos, body, _) ->
                 ...
                 |> Function_table.add_body
                      ...
                      ~function_name);
      if config.Dce_config.cli.debug then Function_table.dump function_table;
