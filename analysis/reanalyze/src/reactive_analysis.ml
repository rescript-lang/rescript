...
type t =
  (Cmt_format.cmt_infos, cmt_file_result option) Reactive_file_collection.t
...
let create ~config : t =
  Reactive_file_collection.create ~read_file:Cmt_format.read_cmt
...
let process_files ~(collection : t) ~config:_ cmt_file_paths :
  ...
  Timing.time_phase `FileLoading (fun () ->
      ...
      let cached_before =
        cmt_file_paths
        |> List.filter (fun p -> Reactive_file_collection.mem collection p)
        |> List.length
      in

      (* Process all files as a batch - emits single Batch delta *)
      let processed =
        Reactive_file_collection.process_files_batch collection cmt_file_paths
      in
...
let to_file_data_collection (collection : t) :
  ...
  Reactive.flat_map ~name:"file_data_collection"
    (Reactive_file_collection.to_collection collection)
