...
function update(t, newK, f, cmp) {
  ...
  if (newD$1 !== undefined) {
    return Belt_internalAVLtree.singleton(newK, newD$1);
  } else {
    return t;
  }
}
...
function merge(s1, s2, f, cmp) {
  if (s1 === undefined) {
    if (s2 !== undefined) {
      return Belt_internalAVLtree.keepMap(s2, (k, v) => f(k, undefined, v));
    } else {
      return;
    }
  }
  if (s2 === undefined) {
    return Belt_internalAVLtree.keepMap(s1, (k, v) => f(k, v, undefined));
  }
  if (s1.h >= s2.h) {
    let v1 = s1.k;
    let d1 = s1.v;
