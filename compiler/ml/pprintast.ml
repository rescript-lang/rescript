...
let view_expr x =
  ...
  | Pexp_construct ({txt = Lident "::"; _}, [_; _]) ->
    let rec loop exp acc =
      ...
      | {
       ...
       pexp_attributes = [];
        ...
        (List.rev acc, true)
      | {
       pexp_desc = Pexp_construct ({txt = Lident "::"; _}, [e1; e2]);
       pexp_attributes = [];
      } ->
        loop e2 (e1 :: acc)
...
and pattern1 ctxt (f : Format.formatter) (x : pattern) : unit =
  let rec pattern_list_helper f = function
    | {
        ppat_desc = Ppat_construct ({txt = Lident "::"; _}, [pat1; pat2]);
        ppat_attributes = [];
      } ->
...
and simple_expr ctxt f x =
  if x.pexp_attributes <> [] then expression ctxt f x
    ...
    | Pexp_for_await_of (s, e1, e2) ->
      let fmt : (_, _, _) format =
        "@[<hv0>@[<hv2>@[<2>for await %a of@;%a@;do@]@;%a@]@;done@]"
      in
      let expression = expression ctxt in
      pp f fmt (pattern ctxt) s expression e1 expression e2
