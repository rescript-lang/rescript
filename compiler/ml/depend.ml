...
let bound = Node (String_set.empty, String_map.empty)
...
let rec lookup_free p m =
  ...
  | [] -> raise Not_found
  | s :: p -> (
    let (Node (f, m')) = String_map.find s m in
...
let open_module bv lid =
  ...
  | Node (s, m) ->
    add_names s;
    String_map.fold String_map.add m bv
  | exception Not_found ->
    add_path bv lid;
    bv
...
let rec add_expr bv exp =
  ...
  | Pexp_constraint (e1, ty2) ->
    add_expr bv e1;
    add_type bv ty2
  | Pexp_object_get (e, _m) -> add_expr bv e
  | Pexp_object_set (e, _m, v) ->
    add_expr bv e;
    add_expr bv v
...
and add_modtype bv mty =
  ...
  | Pmty_functor (id, mty1, mty2) ->
    ...
    add_modtype (String_map.add id.txt bound bv) mty2
  | Pmty_with (mty, cstrl) ->
    add_modtype bv mty;
    List.iter
      (function
...
and add_signature_binding bv sg =
  snd (List.fold_left add_sig_item (bv, String_map.empty) sg)
...
and add_sig_item (bv, m) item =
  ...
  | Psig_module pmd ->
    ...
    let add = String_map.add pmd.pmd_name.txt m' in
    (add bv, add m)
  | Psig_recmodule decls ->
    let add =
      List.fold_right (fun pmd -> String_map.add pmd.pmd_name.txt bound) decls
    in
  ...
  | Psig_include incl ->
    ...
    add_names s;
    let add = String_map.fold String_map.add m' in
    (add bv, add m)
...
and add_module bv modl =
  ...
  | Pmod_functor (id, mty, modl) ->
    ...
    add_module (String_map.add id.txt bound bv) modl
...
and add_structure_binding bv item_list =
  List.fold_left add_struct_item (bv, String_map.empty) item_list

and add_struct_item (bv, m) item : _ String_map.t * _ String_map.t =
  match item.pstr_desc with
  | Pstr_eval (e, _attrs) ->
    add_expr bv e;
    (bv, m)
  ...
  | Pstr_module x ->
    ...
    let add = String_map.add x.pmb_name.txt b in
    (add bv, add m)
  | Pstr_recmodule bindings ->
    let add =
      List.fold_right (fun x -> String_map.add x.pmb_name.txt bound) bindings
    in
    let bv' = add bv and m = add m in
