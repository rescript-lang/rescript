...
let remove_value_binding_duplicates structure_items =
  let rec process_bindings (bindings : Typedtree.value_binding list) ~seen =
    ...
    | ({vb_pat = {pat_desc = Tpat_var (id, _)}} as binding) :: other_bindings ->
      ...
      if !seen |> String_set.mem name then
        other_bindings |> process_bindings ~seen
      else (
        seen := !seen |> String_set.add name;
  ...
  |> process_items ~acc:[] ~seen:(ref String_set.empty)
      ...
      |> List.fold_left
           (fun bound (value_binding : Typedtree.value_binding) ->
             ...
             | {vb_pat = {pat_desc = Tpat_var (id, _)}} ->
               bound |> String_set.add (id |> Ident.name)
             | _ -> bound)
           bound_in_rest
    in
    ( bound,
...
let rec translate_module_binding ~(config : Gen_type_config.t)
  ...
  | Tmod_ident (path, _) -> (
    ...
    type_env |> Type_env.add_module_equation ~dep ~internal;
    match Env.scrape_alias mb_expr.mod_env mb_expr.mod_type with
    | Mty_signature signature ->
      (* Treat module M = N as include N *)
      signature
      |> Translate_signature_from_types.translate_signature_from_types ~config
           ~output_file_relative ~resolver ~type_env
      |> Translation.combine
  ...
  | Tmod_unpack (_, module_type) -> (
    ...
    | Mty_signature signature ->
      ...
      |> Translation.combine
    | Mty_ident path -> (
      match type_env |> Type_env.lookup_module_type_signature ~path with
      | None -> Translation.empty
      | Some (signature, _) ->
        signature
        |> Translate_signature.translate_signature ~config ~output_file_relative
             ~resolver ~type_env
        |> Translation.combine)
    | Mty_functor _ ->
      log_not_implemented ("Mty_functor " ^ __LOC__);
...
and translate_structure_item ~config ~output_file_relative ~resolver ~type_env
  ...
  | {str_desc = Tstr_type (rec_flag, type_declarations)} ->
    {
      ...
      type_declarations =
        type_declarations
        |> Translate_type_declarations.translate_type_declarations ~config
  ...
  | {str_desc = Tstr_modtype module_type_declaration} ->
    ...
    |> Translate_signature.translate_module_type_declaration ~config
         ~output_file_relative ~resolver ~type_env
