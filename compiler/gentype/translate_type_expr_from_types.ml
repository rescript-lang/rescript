...
let translate_constr ~config ~params_translation ~(path : Path.t) ~type_env =
  let default_case () =
    ...
    let type_param_deps =
      ...
      |> List.concat
    in
    match type_env |> Type_env.apply_type_equations ~config ~path with
  ...
  | ["React"; "component"], [props_translation] ->
    {
      ...
      type_ =
        Function
          {
            arg_types = [{a_name = ""; a_type = props_translation.type_}];
            ret_type = Emit_type.type_react_element;
            type_vars = [];
          };
    }
  | ["React"; "Context"; "t"], [param_translation] ->
    {
      dependencies = param_translation.dependencies;
      type_ = Emit_type.type_react_context ~type_:param_translation.type_;
    }
  ...
  | ["ReactDOMRe"; "Ref"; "currentDomRef"], [] ->
    {dependencies = []; type_ = Emit_type.type_any}
  | ["ReactEvent"; "Mouse"; "t"], [] ->
    {dependencies = []; type_ = Emit_type.type_react_event_mouse_t}
  | ( ( ["React"; "element"]
...
and translateTypeExprFromTypes_ ~config ~type_vars_gen ~type_env
  ...
  | Tvar None ->
    let type_name =
      Gen_ident.js_type_name_for_anonymous_type_id ~type_vars_gen type_expr.id
    in
  ...
  | Tobject t_obj ->
    let rec get_field_types (texp : Types.type_expr) =
      ...
      | Tfield {name; mutability; typ = t1; rest = t2} ->
