...
let rec render_type ~(config : Config.t) ?(indent = None)
  ...
  | Nullable type_ ->
    let use_parens x =
      match type_ with
      | Function _ | Variant _ -> Emit_text.parens [x]
      | _ -> x
    in
    "(null | undefined | "
    ^ use_parens
...
and render_field ~config ~indent ~type_name_is_interface ~in_fun_type
  ...
  let def_str =
    ...
    ^ (type_ |> render_type ~config ~indent ~type_name_is_interface ~in_fun_type)
  in
  if Doc_string.has_content doc_string then
    (* Always print comments on newline before definition. *)
    let indent_str = indent |> Option.value ~default:"" in
    "\n" ^ indent_str ^ Doc_string.render doc_string ^ indent_str ^ def_str
...
let emit_export_const ~early ?(comment = "") ~config
    ?(doc_string = Doc_string.empty) ~emitters ~name ~type_
...
let emit_export_type ~(config : Config.t) ~emitters ~name_as ~opaque ~type_
  ...
  let doc_string = Doc_string.render doc_string in
  let type_params_string = Emit_text.generics_string ~type_vars in
...
let emit_require ~imported_value_or_component ~early ~emitters
  ...
  let module_name_string = Module_name.to_string module_name in
  let import_path_string = Import_path.emit import_path in
  let output =
...
let emit_import_type_as ~emitters ~config ~type_name ~as_type_name
  ...
  in
  let import_path_string = import_path |> Import_path.emit in
