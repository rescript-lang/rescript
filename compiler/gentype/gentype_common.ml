...
let label_js_to_string case =
  ...
  | FloatLabel s -> s
  | IntLabel i -> i
  | StringLabel s -> s |> Emit_text.quotes
...
type dep =
  | External of string
  | Internal of Resolved_name.t
  | Dot of dep * string

module Scoped_package =
...
let rec dep_to_string dep =
  match dep with
  | External name -> name |> Scoped_package.remove_generated_module
  | Internal resolved_name -> resolved_name |> Resolved_name.to_string
  | Dot (d, s) -> dep_to_string d ^ "_" ^ s

let rec dep_to_resolved_name (dep : dep) =
  match dep with
  | External name -> name |> Resolved_name.from_string
