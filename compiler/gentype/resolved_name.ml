...
let rec apply_equations_to_elements ~(eqs : eq list) ~seen (elements : t list) :
  ...
  let apply_eqs el =
    let fresh_elements =
      ...
      |> List.concat
      |> List.filter (fun y -> not (Name_set.mem y seen))
    in
  ...
  let new_seen = Name_set.union seen (new_elements |> Name_set.of_list) in
...
let apply_equations ~(eqs : eq list) (el : t) : eq list =
  [el] |> apply_equations_to_elements ~eqs ~seen:Name_set.empty
