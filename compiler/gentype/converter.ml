...
let type_get_inlined ~config ~lookup_id ~type_name_is_interface type0 =
  let circular = ref "" in
  let rec visit ~(visited : String_set.t) type_ =
    ...
    | Ident {builtin = false; name; type_args} -> (
      if visited |> String_set.mem name then (
        circular := name;
        normalized_)
      else
        let visited = visited |> String_set.add name in
        match name |> lookup_id with
        | {Code_item.annotation = GenTypeOpaque} -> normalized_
        ...
        | {type_vars; type_} ->
          ...
          in
          let inlined = type_ |> Type_vars.substitute ~f |> visit ~visited in
          inlined
        | exception Not_found ->
          let type_args =
