...
let input_cmt_translate_type_declarations ~config ~output_file_relative
  ...
  let translations =
    ...
    | Implementation structure ->
      ...
      |> Translate_structure.translate_structure ~config ~output_file_relative
           ~resolver ~type_env
    | Interface signature ->
      {
        signature with
...
let process_cmt_file cmt =
  ...
  if cmt_file <> "" then
    ...
    if has_gentype_annotations then
      ...
      output_file |> Generated_files.log_file_action TypeError
    else (
      output_file |> Generated_files.log_file_action NoMatch;
