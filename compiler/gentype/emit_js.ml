...
type env = {
  ...
  requires: Import_path.t Config.Module_name_map.t;
      (**  For each .cmt we import types from, keep the map of exported types *)
  cmt_to_export_type_map: Code_item.export_type_map String_map.t;
...
let require_module ~import ~env ~import_path module_name =
  ...
  in
  let requires_new =
    requires |> Config.Module_name_map.add module_name import_path
  in
  match import with
...
let create_export_type_map ~config ~file ~from_cmt_read_recursively
  ...
  let update_export_type_map (export_type_map : Code_item.export_type_map)
    ...
    let add_export_type ~annotation
        ({resolved_type_name; type_; type_vars} : Code_item.export_type) =
      let annotation =
      ...
      if !Debug.code_items then
        Log_.item "Type Map: %s%s%s\n"
          ...
          (" "
          ^ (annotation |> Annotation.to_string |> Emit_text.comment)
          ^ " = "
          ^ (type_
            |> Emit_type.type_to_string ~config
                 ~type_name_is_interface:(fun _ -> false)));
      export_type_map
      |> String_map.add
           (resolved_type_name |> Resolved_name.to_string)
           {Code_item.type_vars; type_; annotation}
    in
    match type_declaration.export_from_type_declaration with
    | {export_type; annotation} -> export_type |> add_export_type ~annotation
  in
  type_declarations |> List.fold_left update_export_type_map String_map.empty

let code_item_to_string ~config ~type_name_is_interface
  ...
  | ImportValue {import_annotation} ->
    "ImportValue " ^ (import_annotation.import_path |> Import_path.dump)
...
let emit_export_type ~emitters ~config ~type_name_is_interface
  ...
  let free_type_vars = Type_vars.free type_ in
  let is_gadt =
  ...
  in
  resolved_type_name |> Resolved_name.to_string
  |> Emit_type.emit_export_type ~config ~emitters ~name_as ~opaque ~type_
       ~type_name_is_interface ~type_vars ~doc_string

let type_name_is_interface ~(export_type_map : Code_item.export_type_map)
    ~(export_type_map_from_other_files : Code_item.export_type_map) type_name =
  let type_is_interface type_ =
    match type_ with
    | Object _ -> true
    | _ -> false
  in
  match export_type_map |> String_map.find type_name with
  | {type_} -> type_ |> type_is_interface
  | exception Not_found -> (
    match export_type_map_from_other_files |> String_map.find type_name with
    | {type_} -> type_ |> type_is_interface
    | exception Not_found -> false)

let emit_export_from_type_declaration ~config ~emitters ~env
    ~type_name_is_interface
    (export_from_type_declaration : Code_item.export_from_type_declaration) =
  ( env,
...
let emit_code_item ~config ~emitters ~module_items_emitter ~env ~file_name
  ...
  | ImportValue {as_path; import_annotation; type_; value_name} ->
    ...
    let emitters, imported_as_name, env =
      ...
      let emitters =
        import_path
        |> Emit_type.emit_import_value_as_early ~emitters
    ...
    let type_ =
      ...
      | Function
        ...
        when ret_type |> Emit_type.is_type_function_component ~fields ->
        (* JSX V3 *)
        let fields =
          fields
      ...
      | Function
        ...
        when Filename.check_suffix name "props"
             && ret_type |> Emit_type.is_type_function_component ~fields:[] -> (
        ...
        | Object (closed_flags, fields) ->
          ...
          let fields =
            Ext_list.filter_map fields (fun (field : field) ->
                ...
                | "children" when field.type_ |> Emit_type.is_type_react_element
                  ->
                  Some {field with type_ = Emit_type.type_react_child}
                | "key" ->
    ...
    let emitters =
      ...
      |> Emit_type.emit_type_cast ~config ~type_ ~type_name_is_interface
      |> Emit_type.emit_export_const
           ~comment:
    ...
    let emitters =
      match value_name = "default" with
      | true -> Emit_type.emit_export_default ~emitters value_name_not_default
      | false -> emitters
    in
    ({env with imported_value_or_component = true}, emitters)
  | ExportValue
      {doc_string; module_access_path; original_name; resolved_name; type_} ->
    let resolved_name_str = Resolved_name.to_string resolved_name in
    let import_path =
      file_name
      |> Module_resolver.resolve_module ~config ~import_extension:config.suffix
           ~output_file_relative ~resolver ~use_bs_dependencies:false
    in
    let file_name_js = file_name |> Module_name.for_js_file in
    ...
    in
    let module Hook_type = struct
      type t = {
        props_type: type_;
        resolved_type_name: Resolved_name.t;
      ...
      | Function
        ...
        let resolved_type_name =
          ...
          then (
            ...
            Resolved_name.from_string "Props")
          else Resolved_name.from_string name |> Resolved_name.dot "Props"
        in
        ( Function function_,
          Some {Hook_type.props_type; resolved_type_name; type_vars} )
      | Function
            ...
            let props_type =
              let fields =
                Ext_list.filter_map fields (fun (field : field) ->
                    match field.name_js with
                    | "children"
                      when field.type_ |> Emit_type.is_type_react_element ->
                      Some {field with type_ = Emit_type.type_react_child}
                    | "key" ->
                      (* Filter out key, which is added to the props type definition in V4 *)
                      None
    ...
    in
    let emitters =
      (file_name_js |> Module_name.to_string)
      ^ "."
      ^ (module_access_path |> Runtime.emit_module_access_path ~config)
      |> Emit_type.emit_export_const ~config ~doc_string ~early:false ~emitters
           ~name ~type_ ~type_name_is_interface
    in
    let emitters =
      match original_name = default with
      | true -> Emit_type.emit_export_default ~emitters Runtime.default
      | false -> emitters
    in
...
let emit_requires ~imported_value_or_component ~early ~config ~requires emitters
    =
  Config.Module_name_map.fold
    (fun module_name import_path emitters ->
      import_path
      |> Emit_type.emit_require ~imported_value_or_component ~early ~emitters
           ~config ~module_name)
    requires emitters
...
let rec read_cmt_files_recursively ~config ~env
  ...
  let update_type_map_from_other_files ~as_type ~export_type_map_from_cmt env =
    ...
    | (export_type_item : Code_item.export_type_item) ->
      let type_ =
      ...
      in
      {
        ...
        export_type_map_from_other_files =
          ...
          |> String_map.add as_type {export_type_item with type_};
      }
    | exception Not_found -> env
  in
  let cmt_file =
    import_path
    |> Import_path.to_cmt ~config ~output_file_relative
    |> Paths.get_cmt_file
  in
  match as_type_name with
  | Some as_type when cmt_file <> "" -> (
    match env.cmt_to_export_type_map |> String_map.find cmt_file with
    ...
    | exception Not_found ->
      ...
      let cmt_to_export_type_map =
        ...
        |> String_map.add cmt_file export_type_map_from_cmt
      in
      let env =
  ...
  |> List.filter
       (fun
         ({export_from_type_declaration = {annotation}} :
           Code_item.type_declaration)
       -> annotation <> NoGenType)

let propagate_annotation_to_sub_types ~code_items
    (type_map : Code_item.export_type_map) =
  let annotated_set = ref String_set.empty in
  ...
  let visit_typ_and_update_marked type0 =
    ...
    let rec visit type_ =
      ...
      | Ident {name = type_name; type_args} ->
        if !visited |> String_set.mem type_name then ()
        else (
          visited := !visited |> String_set.add type_name;
  ...
  let new_type_map =
    type_map
    |> String_map.mapi
         (fun type_name (export_type_item : Code_item.export_type_item) ->
           {
             export_type_item with
             annotation =
               (match !annotated_set |> String_set.mem type_name with
...
let emit_translation_as_string ~config ~file_name
  ...
  let initial_env =
    {
      requires = Config.Module_name_map.empty;
      requires_early = Config.Module_name_map.empty;
      cmt_to_export_type_map = String_map.empty;
      export_type_map_from_other_files = String_map.empty;
  ...
  let export_type_map, annotated_set =
    ...
    |> create_export_type_map ~config
         ~file:(file_name |> Module_name.to_string)
  ...
  in
  let lookupId_ ~env s =
    try export_type_map |> String_map.find s
    with Not_found ->
      env.export_type_map_from_other_files |> String_map.find s
  in
  let emitters = Emitters.initial
  and module_items_emitter = Export_module.create_module_items_emitter ()
  ...
  let emitters =
    ...
    | true -> Emit_type.emit_import_react ~emitters
    | false -> emitters
  in
  let env =
    ...
    | true ->
      ...
      |> require_module ~import:true ~env
           ~import_path:(Import_path.bs_curry_path ~config)
    | false -> env
  in
  let final_env = env in
  let emitters =
    module_items_emitter
    |> Export_module.emit_all_module_items ~config ~emitters ~file_name
  in
  emitters
