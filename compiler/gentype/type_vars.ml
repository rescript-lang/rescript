...
let rec free_ type0 : String_set.t =
  ...
  | Function {arg_types; ret_type; type_vars} ->
    String_set.diff
      ...
      (type_vars |> String_set.of_list)
  | Object (_, fields) ->
    fields
    |> List.fold_left
         (fun s {type_} -> String_set.union s (type_ |> free_))
         String_set.empty
  | Ident {type_args} ->
    type_args
    |> List.fold_left
         (fun s type_arg -> String_set.union s (type_arg |> free_))
         String_set.empty
  ...
  | Tuple inner_types ->
    ...
    |> List.fold_left
         (fun s type_arg -> String_set.union s (type_arg |> free_))
         String_set.empty
  | TypeVar s -> s |> String_set.singleton
  | Variant {payloads} ->
    payloads
    |> List.fold_left
         (fun s {t} -> String_set.union s (t |> free_))
         String_set.empty

and freeOfList_ types =
  types
  |> List.fold_left (fun s {a_type} -> s +++ (a_type |> free_)) String_set.empty

and ( +++ ) = String_set.union

let free type_ = type_ |> free_ |> String_set.elements
