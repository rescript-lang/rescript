...
type t = {
  mutable map: entry String_map.t;
  mutable map_module_types: (Typedtree.signature * t) String_map.t;
  mutable module_equation: module_equation option;
  mutable module_item: Runtime.module_item;
  name: string;
  parent: t option;
  type_equations: type_ String_map.t;
}
...
let create_type_env ~name parent =
  let module_item = Runtime.new_module_item ~name in
  {
    ...
    map_module_types = String_map.empty;
    module_equation = None;
    module_item;
    name;
    parent;
    type_equations = String_map.empty;
  }
...
let rec add_type_equation ~flattened ~type_ type_env =
  ...
  | [name] ->
    {
      type_env with
      type_equations = type_env.type_equations |> String_map.add name type_;
    }
  | module_name :: rest -> (
...
let rec add_module_path ~type_env name =
  match type_env.parent with
  | None -> name |> Resolved_name.from_string
  | Some parent ->
    type_env.name |> add_module_path ~type_env:parent |> Resolved_name.dot name

let rec get_module_equations type_env : Resolved_name.eq list =
  let sub_equations =
