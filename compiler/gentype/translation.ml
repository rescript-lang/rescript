...
let combine (translations : t list) : t =
  ...
  {
    Code_item.import_types = import_types |> List.concat;
...
let dep_to_import_type ~config ~output_file_relative ~resolver (dep : dep) =
  ...
  | External name when name = "list" ->
    [
      {
        Code_item.type_name = "list";
        as_type_name = None;
        import_path =
          Module_name.rescript_pervasives
          |> Module_resolver.import_path_for_reason_module_name ~config
...
let translate_value ~attributes ~config ~doc_string ~output_file_relative
  ...
  let type_expr_translation =
    ...
    |> Translate_type_expr_from_types.translate_type_expr_from_types ~config
         ~type_env
  in
  let type_vars = type_expr_translation.type_ |> Type_vars.free in
  let type_ =
  ...
  let resolved_name_original =
    name |> Type_env.add_module_path ~type_env |> Resolved_name.to_string
  in
  let resolved_name = name_as |> Type_env.add_module_path ~type_env in
  let module_access_path =
    type_env |> Type_env.get_module_access_path ~name:resolved_name_original
  in
  let code_items =
    [
      Code_item.ExportValue
        {
          doc_string;
          module_access_path;
          original_name = name;
...
let add_type_declarations_from_module_equations ~type_env (translation : t) =
  let eqs = type_env |> Type_env.get_module_equations in
