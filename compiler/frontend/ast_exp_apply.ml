...
let app_exp_mapper (e : exp) (self : Ast_mapper.mapper) : exp =
  ...
  | Some {op = "->"; args = [a_; f_]; loc} -> (
    ...
    | Pexp_variant (label, []) ->
      {f with pexp_desc = Pexp_variant (label, [a]); pexp_loc = e.pexp_loc}
    | Pexp_construct (ctor, []) ->
      {f with pexp_desc = Pexp_construct (ctor, [a]); pexp_loc = e.pexp_loc}
              ...
              Pexp_tuple
                (Ext_list.map xs (fun fn ->
                     ...
                     | Pexp_construct (ctor, []) ->
                       {
                         fn with
                         pexp_desc = Pexp_construct (ctor, [bounded_obj_arg]);
                       }
