...
let pos_to_json (p : Lexing.position) =
  Ext_json_noloc.kvs
    [ ("line", Ext_json_noloc.flo (string_of_int p.pos_lnum));
      ( "column",
        Ext_json_noloc.flo (string_of_int (p.pos_cnum - p.pos_bol)) ) ]

let loc_to_json (loc : Location.t) =
  Ext_json_noloc.kvs
    [ ("start", pos_to_json loc.loc_start); (
        "end",
        pos_to_json loc.loc_end ) ]
  ...
  if not (is_enabled ()) then ()
    ...
    let rec walk_mod (m : module_expr) (context_for_mod : string option) =
      ...
      | Pmod_extension (({ txt = tag; loc = _ }, payload))
        ...
        | Some (txt, loc) ->
          let context =
            match context_for_mod with Some c -> c | None -> "module"
          in
      ...
      | Pmod_apply (m1, m2) -> (walk_mod m1 None; walk_mod m2 None)
      | _ -> ()
    and walk_str (s : structure) =
      List.iter
        (fun (si : structure_item) ->
          ...
          | Pstr_recmodule mbs ->
            List.iter (fun ({ pmb_expr; _ } : module_binding) ->
                walk_mod pmb_expr None) mbs
    ...
    let iter : Ast_iterator.iterator =
      ...
      { default_it with
        expr =
          (fun self e ->
            (match e.pexp_desc with
            | Pexp_extension (({txt = tag; _}, payload)) when should_collect_tag tag -> (
              match string_lit_of_payload payload with
