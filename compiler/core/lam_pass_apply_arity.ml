...
let normalize (meta : Lam_stats.t) (lam : Lam.t) : Lam.t =
  let rec populate_apply_info ?(ap_transformed_jsx = false)
    ...
    | x :: _ ->
      if x = len then
        ...
        Lam_eta_conversion.transform_under_supply (x - len) ap_info fn args
      else
        let first, rest = Ext_list.split_at args x in
        Lam.apply ~ap_transformed_jsx
          (Lam.apply (simpl fn) (Ext_list.map first simpl) ap_info)
  ...
  and simpl (lam : Lam.t) =
    ...
    | Lfunction {arity; params; body; attr; loc} ->
      ...
      Lam.function_ ~loc ~arity ~params ~body:(simpl body) ~attr
    | Lswitch
        ( l,
          {
            sw_failaction;
