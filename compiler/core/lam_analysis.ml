...
let not_zero_constant (x : Lambda.structured_constant) =
  match x with
  | Const_int i -> i <> 0l
...
let rec no_side_effects (lam : Lambda.t) : bool =
  match lam with
...
*)
let destruct_pattern (body : Lambda.t) params args =
...
*)
let safe_to_inline (lam : Lambda.t) =
  match lam with
  | Lfunction _ -> true
  | Lconst
