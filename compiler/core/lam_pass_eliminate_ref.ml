...
let rec eliminate_ref id (lam : Lam.t) =
  ...
  *)
  ...
  | Lconst _ -> lam
  | Lapply
      {ap_func = e1; ap_args = el; ap_info; ap_transformed_jsx; ap_result_type}
    ->
    Lam.apply ~ap_transformed_jsx ~ap_result_type (eliminate_ref id e1)
      (Ext_list.map el (eliminate_ref id))
      ap_info
  | Llet (str, v, ty, e1, e2) ->
    Lam.let_ str v ty (eliminate_ref id e1) (eliminate_ref id e2)
  | Lletrec (idel, e2) ->
    Lam.letrec
