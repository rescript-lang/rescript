...
let rewrite (map : _ Hash_ident.t) (lam : Lambda.t) : Lambda.t =
  ...
  and aux (lam : Lambda.t) : Lambda.t =
    ...
    | Lfunction {params; body; attr; loc} ->
      ...
      let body = aux body in
      Lambda.function_ ~loc ~params ~body ~attr
    | Lstaticcatch (l1, (i, xs), l2) ->
      let l1 = aux l1 in
      let xs = Ext_list.map xs rebind in
      let l2 = aux l2 in
      Lambda.staticcatch l1 (i, xs) l2
    | Lfor (ident, l1, l2, dir, l3) ->
      let ident = rebind ident in
      let l1 = aux l1 in
      let l2 = aux l2 in
      let l3 = aux l3 in
      Lambda.for_ ident (aux l1) l2 dir l3
    | Lfor_of (ident, l1, l2) ->
      let ident = rebind ident in
      let l1 = aux l1 in
      let l2 = aux l2 in
      Lambda.for_of ident l1 l2
    | Lfor_await_of (ident, l1, l2) ->
      let ident = rebind ident in
      let l1 = aux l1 in
      let l2 = aux l2 in
      Lambda.for_await_of ident l1 l2
    | Lconst _ -> lam
    ...
    | Lapply {ap_func; ap_args; ap_info; ap_transformed_jsx} ->
      ...
      Lambda.apply ~ap_transformed_jsx fn args ap_info
    | Lswitch
        ( l,
          {
            sw_failaction;
    ...
    | Lstringswitch (l, sw, d) ->
      let l = aux l in
      Lambda.stringswitch l (Ext_list.map_snd sw aux) (option_map d)
    ...
    | Lwhile (l1, l2) ->
      ...
      let l2 = aux l2 in
      Lambda.while_ l1 l2
    | Lassign (v, l) -> Lambda.assign v (aux l)
  in
  aux lam
