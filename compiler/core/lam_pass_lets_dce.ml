  ...
  let rec simplif (lam : Lambda.t) =
      ...
      *)
      ...
      | ( _,
          ( Lconst
              ...
              | Const_char _ | Const_float _ | Const_bigint _ | Const_polyvar _
    ...
    | Llet (((Strict | Variable) as kind), v, l1, l2) -> (
      if not (used v) then
        ...
        if Lam_analysis.no_side_effects l1 then l2 else Lambda.seq l1 l2
      else
        let l1 = simplif l1 in
    ...
    | Lprim {primitive = Pstringadd; args = [l; r]; loc} -> (
      ...
      | Some l_s -> (
        ...
        match opt_r with
        | None -> Lambda.prim ~primitive:Pstringadd ~args:[l'; r'] loc
        | Some r_s -> Lambda.const (Const_string {s = l_s ^ r_s; delim = None}))
      )
