...
let lambda_as_module
  ...
  if Js_packages_info.is_empty package_info && !Js_config.js_stdout then begin
    (* Stdout mode: used by "rewatch compile-file" for one-shot compilation.
       No -bs-package-output was provided, so we output JS to stdout using
       the module system configured via -bs-module-system (or default). *)
