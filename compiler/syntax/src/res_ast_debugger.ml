...
(* SexpAst with location information - for debugging position parity *)
module SexpAstWithLocs = struct
  open Parsetree
  ...
  let arg_label_loc lbl =
    match lbl with
    | Asttypes.Nolabel -> Sexp.atom "Nolabel"
    | Labelled {txt; loc} -> Sexp.list [Sexp.atom "Labelled"; string txt; location loc]
    | Optional {txt; loc} -> Sexp.list [Sexp.atom "Optional"; string txt; location loc]

  let arity (a : Asttypes.arity) =
    match a with
    | Some n -> Sexp.list [Sexp.atom "Some"; Sexp.atom (string_of_int n)]
    | None -> Sexp.atom "None"

  let position_opt pos =
    match pos with
    | None -> Sexp.atom "None"
    | Some p ->
      ...
      Sexp.list [
        ...
        Sexp.list [
          Sexp.atom "pos";
          Sexp.atom (string_of_int p.Lexing.pos_lnum);
          Sexp.atom (string_of_int col);
        ];
      ]
  ...
  and structure_item si =
    let desc =
      ...
      | Pstr_extension (ext, attrs) ->
        Sexp.list [Sexp.atom "Pstr_extension"; extension ext; attributes attrs]
    in
    Sexp.list [Sexp.atom "structure_item"; location si.pstr_loc; desc]
  ...
  and open_description od =
    Sexp.list
      [
        ...
        longident od.popen_lid.Asttypes.txt;
        location od.popen_lid.Asttypes.loc;
  ...
  and module_type_declaration mtd =
    Sexp.list
      [
        ...
        string mtd.pmtd_name.Asttypes.txt;
        location mtd.pmtd_name.Asttypes.loc;
        (match mtd.pmtd_type with
        | None -> Sexp.atom "None"
        | Some mod_type -> Sexp.list [Sexp.atom "Some"; module_type mod_type]);
        attributes mtd.pmtd_attributes;
  ...
  and module_binding mb =
    Sexp.list
      [
        ...
        location mb.pmb_loc;
        string mb.pmb_name.Asttypes.txt;
        location mb.pmb_name.Asttypes.loc;
        module_expression mb.pmb_expr;
        attributes mb.pmb_attributes;
      ]
  ...
  and module_expression me =
    let desc =
      match me.pmod_desc with
      | Pmod_ident mod_name ->
        Sexp.list [Sexp.atom "Pmod_ident"; longident mod_name.Asttypes.txt; location mod_name.Asttypes.loc]
      | Pmod_structure s -> Sexp.list [Sexp.atom "Pmod_structure"; structure s]
      | Pmod_functor (lbl, opt_mod_type, mod_expr) ->
        Sexp.list
          [
      ...
      | Pmod_extension ext ->
        Sexp.list [Sexp.atom "Pmod_extension"; extension ext]
    in
    Sexp.list [Sexp.atom "module_expr"; location me.pmod_loc; desc; attributes me.pmod_attributes]

  and module_type mt =
    let desc =
      match mt.pmty_desc with
      | Pmty_ident longident_loc ->
        Sexp.list [Sexp.atom "Pmty_ident"; longident longident_loc.Asttypes.txt; location longident_loc.Asttypes.loc]
      | Pmty_signature s -> Sexp.list [Sexp.atom "Pmty_signature"; signature s]
      | Pmty_functor (lbl, opt_mod_type, mod_type) ->
        Sexp.list
          [
      ...
      | Pmty_alias longident_loc ->
        Sexp.list [Sexp.atom "Pmty_alias"; longident longident_loc.Asttypes.txt; location longident_loc.Asttypes.loc]
      | Pmty_extension ext ->
        Sexp.list [Sexp.atom "Pmty_extension"; extension ext]
      | Pmty_typeof mod_expr ->
        Sexp.list [Sexp.atom "Pmty_typeof"; module_expression mod_expr]
    ...
    in
    Sexp.list [Sexp.atom "module_type"; location mt.pmty_loc; desc; attributes mt.pmty_attributes]
    ...
    | Pwith_type (longident_loc, td) ->
      Sexp.list
        [
          Sexp.atom "Pmty_with";
          longident longident_loc.Asttypes.txt;
          location longident_loc.Asttypes.loc;
          type_declaration td;
        ]
    | Pwith_module (l1, l2) ->
      Sexp.list
        [
          Sexp.atom "Pwith_module";
          longident l1.Asttypes.txt;
          location l1.Asttypes.loc;
          longident l2.Asttypes.txt;
          location l2.Asttypes.loc;
        ]
    | Pwith_typesubst (longident_loc, td) ->
      Sexp.list
        [
  ...
  and signature_item si =
    let descr =
      ...
      | Psig_extension (ext, attrs) ->
        Sexp.list [Sexp.atom "Psig_extension"; extension ext; attributes attrs]
    in
    Sexp.list [Sexp.atom "signature_item"; location si.psig_loc; descr]
  ...
  and module_declaration md =
    Sexp.list
      [
        ...
        location md.pmd_loc;
        string md.pmd_name.Asttypes.txt;
        location md.pmd_name.Asttypes.loc;
        module_type md.pmd_type;
        attributes md.pmd_attributes;
      ]
  ...
  and value_description vd =
    Sexp.list
      [
        ...
        string vd.pval_name.Asttypes.txt;
        location vd.pval_name.Asttypes.loc;
        core_type vd.pval_type;
        Sexp.list (map_empty ~f:string vd.pval_prim);
        attributes vd.pval_attributes;
      ]
  ...
  and type_declaration td =
    Sexp.list
      [
        ...
        string td.ptype_name.Asttypes.txt;
        location td.ptype_name.Asttypes.loc;
        Sexp.list
          [
            Sexp.atom "ptype_params";
            Sexp.list
        ...
        Sexp.list
          [
            ...
            Sexp.list
              (map_empty
                 ~f:(fun (typ1, typ2, loc) ->
                   Sexp.list [core_type typ1; core_type typ2; location loc])
                 td.ptype_cstrs);
          ];
        Sexp.list [Sexp.atom "ptype_kind"; type_kind td.ptype_kind];
        Sexp.list
  ...
  and extension_constructor ec =
    Sexp.list
      [
        ...
        string ec.pext_name.Asttypes.txt;
        location ec.pext_name.Asttypes.loc;
        extension_constructor_kind ec.pext_kind;
        attributes ec.pext_attributes;
      ]
  ...
  and extension_constructor_kind kind =
    ...
    | Pext_rebind longident_loc ->
      Sexp.list [Sexp.atom "Pext_rebind"; longident longident_loc.Asttypes.txt; location longident_loc.Asttypes.loc]
  ...
  and type_extension te =
    Sexp.list
      [
        Sexp.atom "type_extension";
        Sexp.list
          [Sexp.atom "ptyext_path"; longident te.ptyext_path.Asttypes.txt; location te.ptyext_path.Asttypes.loc];
        Sexp.list
          [
            Sexp.atom "ptyext_parms";
            Sexp.list
  ...
  and constructor_declaration cd =
    Sexp.list
      [
        ...
        location cd.pcd_loc;
        string cd.pcd_name.Asttypes.txt;
        location cd.pcd_name.Asttypes.loc;
        Sexp.list [Sexp.atom "pcd_args"; constructor_arguments cd.pcd_args];
        Sexp.list
          [
            Sexp.atom "pcd_res";
  ...
  and label_declaration ld =
    Sexp.list
      [
        ...
        location ld.pld_loc;
        string ld.pld_name.Asttypes.txt;
        location ld.pld_name.Asttypes.loc;
        mutable_flag ld.pld_mutable;
        Sexp.atom (if ld.pld_optional then "true" else "false");
        core_type ld.pld_type;
        attributes ld.pld_attributes;
      ]
  ...
  and expression expr =
    let desc =
      match expr.pexp_desc with
      | Pexp_ident longident_loc ->
        Sexp.list [Sexp.atom "Pexp_ident"; longident longident_loc.Asttypes.txt; location longident_loc.Asttypes.loc]
      | Pexp_constant c -> Sexp.list [Sexp.atom "Pexp_constant"; constant c]
      | Pexp_let (flag, vbs, expr) ->
        Sexp.list
          [
      ...
      | Pexp_fun
          {arg_label = arg_lbl; default = expr_opt; lhs = pat; rhs = expr; arity = ar; async} ->
        Sexp.list
          [
            Sexp.atom "Pexp_fun";
            arg_label_loc arg_lbl;
            ...
            expression expr;
            arity ar;
            Sexp.atom (if async then "true" else "false");
          ]
      | Pexp_apply {funct = expr; args; partial; transformed_jsx} ->
        Sexp.list
          [
            Sexp.atom "Pexp_apply";
            expression expr;
            Sexp.list
              (map_empty
                 ~f:(fun (arg_lbl, expr) ->
                   Sexp.list [arg_label_loc arg_lbl; expression expr])
                 args);
            Sexp.atom (if partial then "true" else "false");
            Sexp.atom (if transformed_jsx then "true" else "false");
          ]
      | Pexp_match (expr, cases) ->
        Sexp.list
          [
      ...
      | Pexp_record (rows, opt_expr) ->
        Sexp.list
          [
            ...
            Sexp.list
              (map_empty
                 ~f:(fun {lid = longident_loc; x = expr; opt} ->
                   Sexp.list
                     [longident longident_loc.Asttypes.txt; location longident_loc.Asttypes.loc; expression expr; Sexp.atom (if opt then "true" else "false")])
                 rows);
            (match opt_expr with
            | None -> Sexp.atom "None"
            | Some expr -> Sexp.list [Sexp.atom "Some"; expression expr]);
      ...
      | Pexp_coerce (expr, (), typexpr) ->
        Sexp.list [Sexp.atom "Pexp_coerce"; expression expr; core_type typexpr]
      | Pexp_send (expr, {txt = method_name; loc}) ->
        Sexp.list [Sexp.atom "Pexp_send"; string method_name; location loc; expression expr]
      | Pexp_letmodule (mod_name, mod_expr, expr) ->
        Sexp.list
          [
            Sexp.atom "Pexp_letmodule";
            string mod_name.Asttypes.txt;
            location mod_name.Asttypes.loc;
            module_expression mod_expr;
            expression expr;
          ]
      | Pexp_letexception (ext_constr, expr) ->
      ...
      | Pexp_newtype (lbl, expr) ->
        Sexp.list
          [Sexp.atom "Pexp_newtype"; string lbl.Asttypes.txt; location lbl.Asttypes.loc; expression expr]
      | Pexp_pack mod_expr ->
        Sexp.list [Sexp.atom "Pexp_pack"; module_expression mod_expr]
      | Pexp_open (flag, longident_loc, expr) ->
        Sexp.list
    ...
    in
    Sexp.list [Sexp.atom "expression"; location expr.pexp_loc; desc; attributes expr.pexp_attributes]

  and jsx_prop = function
    | JSXPropPunning (_is_optional, name) -> Sexp.list [Sexp.atom name.txt; location name.loc]
    | JSXPropValue (name, _is_optional, expr) ->
      Sexp.list [Sexp.atom name.txt; location name.loc; expression expr]
    | JSXPropSpreading (loc, expr) -> Sexp.list [Sexp.atom "..."; location loc; expression expr]

  and case c =
    Sexp.list
      [
        Sexp.atom "case";
        Sexp.list [Sexp.atom "pc_bar"; position_opt c.pc_bar];
        Sexp.list [Sexp.atom "pc_lhs"; pattern c.pc_lhs];
        Sexp.list
          [
            Sexp.atom "pc_guard";
  ...
  and pattern p =
    let descr =
      ...
      | Ppat_any -> Sexp.atom "Ppat_any"
      | Ppat_var var ->
        Sexp.list [Sexp.atom "Ppat_var"; string var.Location.txt; location var.Location.loc]
      | Ppat_alias (p, alias) ->
        Sexp.list [Sexp.atom "Ppat_alias"; pattern p; string alias.txt; location alias.loc]
      | Ppat_constant c -> Sexp.list [Sexp.atom "Ppat_constant"; constant c]
      | Ppat_interval (lo, hi) ->
        Sexp.list [Sexp.atom "Ppat_interval"; constant lo; constant hi]
      | Ppat_tuple patterns ->
      ...
      | Ppat_construct (longident_loc, opt_pattern) ->
        Sexp.list
          [
            Sexp.atom "Ppat_construct";
            longident longident_loc.Location.txt;
            location longident_loc.Location.loc;
            (match opt_pattern with
            | None -> Sexp.atom "None"
            | Some p -> Sexp.list [Sexp.atom "some"; pattern p]);
          ]
      ...
      | Ppat_record (rows, flag) ->
        Sexp.list
          [
            ...
            Sexp.list
              (map_empty
                 ~f:(fun {lid = longident_loc; x = p; opt} ->
                   Sexp.list [longident longident_loc.Location.txt; location longident_loc.Location.loc; pattern p; Sexp.atom (if opt then "true" else "false")])
                 rows);
          ]
      | Ppat_array patterns ->
        Sexp.list
      ...
      | Ppat_constraint (p, typexpr) ->
        Sexp.list [Sexp.atom "Ppat_constraint"; pattern p; core_type typexpr]
      | Ppat_type longident_loc ->
        Sexp.list [Sexp.atom "Ppat_type"; longident longident_loc.Location.txt; location longident_loc.Location.loc]
      | Ppat_unpack string_loc ->
        Sexp.list [Sexp.atom "Ppat_unpack"; string string_loc.Location.txt; location string_loc.Location.loc]
      | Ppat_exception p -> Sexp.list [Sexp.atom "Ppat_exception"; pattern p]
      | Ppat_extension ext ->
        Sexp.list [Sexp.atom "Ppat_extension"; extension ext]
      | Ppat_open (longident_loc, p) ->
    ...
    in
    Sexp.list [Sexp.atom "pattern"; location p.ppat_loc; descr; attributes p.ppat_attributes]
  ...
  and package_type (mod_name_loc, package_constraints) =
    Sexp.list
      [
        Sexp.atom "package_type";
        longident mod_name_loc.Asttypes.txt;
        location mod_name_loc.Asttypes.loc;
        Sexp.list
          (map_empty
             ~f:(fun (mod_name_loc, typexpr) ->
               Sexp.list
                 [longident mod_name_loc.Asttypes.txt; location mod_name_loc.Asttypes.loc; core_type typexpr])
             package_constraints);
      ]
    ...
    let desc =
      ...
      | Ptyp_poly (lbls, typexpr) ->
        Sexp.list
          [
            Sexp.atom "Ptyp_poly";
            Sexp.list (map_empty ~f:(fun lbl -> Sexp.list [string lbl.Asttypes.txt; location lbl.Asttypes.loc]) lbls);
            core_type typexpr;
          ]
      | Ptyp_package package ->
        Sexp.list [Sexp.atom "Ptyp_package"; package_type package]
      | Ptyp_extension ext ->
        Sexp.list [Sexp.atom "Ptyp_extension"; extension ext]
    in
    Sexp.list [Sexp.atom "core_type"; location typexpr.ptyp_loc; desc; attributes typexpr.ptyp_attributes]
  ...
  and attribute (string_loc, p) =
    Sexp.list
      [Sexp.atom "attribute"; Sexp.atom string_loc.Asttypes.txt; location string_loc.Asttypes.loc; payload p]

  and extension (string_loc, p) =
    Sexp.list
      [Sexp.atom "extension"; Sexp.atom string_loc.Asttypes.txt; location string_loc.Asttypes.loc; payload p]
...
let sexp_locs_print_engine = SexpAstWithLocs.print_engine

(* SexpAst0WithLocs - serializes parsetree0 (frozen PPX-compatible format) to sexp with locations *)
module SexpAst0WithLocs = struct
  open Parsetree0
  ...
  (* parsetree0 uses Noloc.arg_label - no location info *)
  let arg_label lbl =
    match lbl with
    | Asttypes.Noloc.Nolabel -> Sexp.atom "Nolabel"
    | Asttypes.Noloc.Labelled txt -> Sexp.list [Sexp.atom "Labelled"; string txt]
    | Asttypes.Noloc.Optional txt -> Sexp.list [Sexp.atom "Optional"; string txt]
  ...
  and signature_item si =
    let descr =
      ...
      | Psig_extension (ext, attrs) ->
        Sexp.list [Sexp.atom "Psig_extension"; extension ext; attributes attrs]
      | Psig_class _ -> Sexp.atom "Psig_class"
      | Psig_class_type _ -> Sexp.atom "Psig_class_type"
    in
    Sexp.list [Sexp.atom "signature_item"; location si.psig_loc; descr]
  ...
  and expression expr =
    let desc =
      ...
      | Pexp_fun (arg_lbl, expr_opt, pat, body) ->
        Sexp.list
          [
            Sexp.atom "Pexp_fun";
            arg_label arg_lbl;
        ...
        Sexp.list
          [
            ...
            Sexp.list
              (map_empty
                 ~f:(fun (lbl, e) ->
                   Sexp.list [arg_label lbl; expression e])
                 args);
          ]
      | Pexp_new _ -> Sexp.atom "Pexp_new"
      | Pexp_setinstvar _ -> Sexp.atom "Pexp_setinstvar"
      | Pexp_override _ -> Sexp.atom "Pexp_override"
      | Pexp_lazy e -> Sexp.list [Sexp.atom "Pexp_lazy"; expression e]
      | Pexp_poly _ -> Sexp.atom "Pexp_poly"
      | Pexp_object _ -> Sexp.atom "Pexp_object"
      | Pexp_match (expr, cases) ->
        Sexp.list
          [
            Sexp.atom "Pexp_match";
            ...
            Sexp.list
              (map_empty
                 ~f:(fun (longident_loc, expr) ->
                   Sexp.list
                     [longident longident_loc.Asttypes.txt; location longident_loc.Asttypes.loc; expression expr])
                 rows);
            (match opt_expr with
            | None -> Sexp.atom "None"
            | Some expr -> Sexp.list [Sexp.atom "Some"; expression expr]);
            ...
            Sexp.list
              (map_empty
                 ~f:(fun (longident_loc, p) ->
                   Sexp.list [longident longident_loc.Location.txt; location longident_loc.Location.loc; pattern p])
                 rows);
          ]
      | Ppat_array patterns ->
        Sexp.list
  ...
  and core_type typexpr =
    let desc =
      ...
      | Ptyp_any -> Sexp.atom "Ptyp_any"
      | Ptyp_var var -> Sexp.list [Sexp.atom "Ptyp_var"; string var]
      | Ptyp_arrow (lbl, arg_typ, ret_typ) ->
        Sexp.list
          [
            Sexp.atom "Ptyp_arrow";
            arg_label lbl;
      ...
      | Ptyp_class () -> Sexp.atom "Ptyp_class"
      | Ptyp_variant (fields, flag, opt_labels) ->
        Sexp.list
          [
            Sexp.atom "Ptyp_variant";
    ...
    Res_driver.
      {
        print_implementation =
          (fun ~width:_ ~filename:_ ~comments:_ parsetree ->
            let tree0 = Ast_mapper_to0.default_mapper.structure
              Ast_mapper_to0.default_mapper parsetree in
            tree0 |> structure |> Sexp.to_string |> print_string);
        print_interface =
          (fun ~width:_ ~filename:_ ~comments:_ parsetree ->
            let tree0 = Ast_mapper_to0.default_mapper.signature
              Ast_mapper_to0.default_mapper parsetree in
            tree0 |> signature |> Sexp.to_string |> print_string);
      }
end

let sexp0_locs_print_engine = SexpAst0WithLocs.print_engine
