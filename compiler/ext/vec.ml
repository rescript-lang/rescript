...
*)
  let to_list d =
    let rec loop (d_arr : elt array) idx accum =
      if idx < 0 then accum
      else loop d_arr (idx - 1) (Array.unsafe_get d_arr idx :: accum)
    in
    loop d.arr (d.len - 1) []
 ...
 *)
  ...
  let fold_left f x a =
    let rec loop a_len (a_arr : elt array) idx x =
      if idx >= a_len then x
      else loop a_len a_arr (idx + 1) (f x (Array.unsafe_get a_arr idx))
    in
...
*)
  ...
  let equal eq x y : bool =
    if x.len <> y.len then false
      ...
      let rec aux x_arr y_arr i =
        if i < 0 then true
        else if eq (Array.unsafe_get x_arr i) (Array.unsafe_get y_arr i) then
          aux x_arr y_arr (i - 1)
        else false
      in
  ...
  *)
  ...
  let push (d : t) v =
    ...
    if d_arr_len = 0 then (
      ...
      if d_len = d_arr_len then (
        ...
        let new_capacity =
          min Sys.max_array_length d_len * 2
  ...
  let delete_range (d : t) idx len =
    let d_len = d.len in
    if len < 0 || idx < 0 || idx + len > d_len then
      invalid_arg "Vec.delete_range";
    let arr = d.arr in
  ...
  let get_and_delete_range (d : t) idx len : t =
    let d_len = d.len in
    if len < 0 || idx < 0 || idx + len > d_len then
      invalid_arg "Vec.get_and_delete_range";
    let arr = d.arr in
  ...
  let inplace_filter f (d : t) : unit =
    ...
    for i = 0 to d_len - 1 do
      ...
      if f x then (
        let curr_p = !p in
        if curr_p <> i then Array.unsafe_set d_arr curr_p x;
        incr p)
    done;
    let last = !p in
